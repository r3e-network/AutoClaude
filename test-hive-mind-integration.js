...
console.log(`   Commands: ${commandTestsPassed}/${requiredCommands.length} ✅\n`);
...
console.log(`   Configurations: ${configTestsPassed}/${requiredConfigs.length} ✅\n`);
...
const agentCapabilities = {
    ...
    'ResearcherAgent': ['code-analysis', 'pattern-discovery', 'best-practices-research'],
    'SecurityAgent': ['vulnerability-scanning', 'security-audit', 'threat-modeling'],
    'DocumentationAgent': ['api-documentation', 'code-documentation', 'user-guides'],
    'OptimizationAgent': ['performance-optimization', 'code-optimization', 'memory-optimization']
};
...
for (const [agentName, capabilities] of Object.entries(agentCapabilities)) {
    ...
    if (fs.existsSync(agentFile)) {
        ...
        if (agentCapabilitiesFound === capabilities.length) {
            console.log(`   ✅ ${agentName}: ${agentCapabilitiesFound}/${capabilities.length} capabilities`);
            capabilityTestsPassed++;
        } else {
            console.log(`   ⚠️  ${agentName}: ${agentCapabilitiesFound}/${capabilities.length} capabilities`);
        }
    }
}

console.log(`   Agent Capabilities: ${capabilityTestsPassed}/${Object.keys(agentCapabilities).length} ✅\n`);
...
const allTests = [
    ...
    { name: 'Commands', passed: commandTestsPassed, total: requiredCommands.length },
    { name: 'Configuration', passed: configTestsPassed, total: requiredConfigs.length },
    ...
    { name: 'Agent Capabilities', passed: capabilityTestsPassed, total: Object.keys(agentCapabilities).length }
];
...
if (overallPercentage === 100) {
    ...
    console.log('⚠️  SYSTEM MOSTLY READY. Review failing tests before deployment.');
} else {
    console.log('❌ SYSTEM NOT READY. Address critical issues before deployment.');
}
