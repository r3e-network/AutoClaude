...
} from "../../../src/agents/AgentCoordinator";
import type {
  Task,
  TaskType,
  AgentType,
...
describe("AgentCoordinator", () => {
  ...
  describe("agent management", () => {
    it("should get agent status", async () => {
      ...
      for (const agent of status) {
        ...
        expect(["idle", "busy", "error", "stopped", "initializing"]).toContain(
          agent.status,
        );
      }
    });
  ...
  describe("performance monitoring", () => {
    ...
    it("should track agent activity", async () => {
      ...
      const hasActivity = agentStatus.some(
        (agent) =>
  ...
  describe("integration scenarios", () => {
    it("should handle complex workflow", async () => {
      ...
      const validationTask = {
        ...
        input: {
          ...
          originalContent:
            "public class Workflow { public int Count { get; set; } }",
    ...
    it("should handle agent coordination under load", async () => {
      ...
      const tasks = Array.from({ length: 10 }, (_, i) => ({
        type: (i % 2 === 0
          ? "convert-file"
          : "validate-conversion") as TaskType,
