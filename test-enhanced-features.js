...
console.log('🧪 Testing Enhanced Features - Universal Language Conversion System\n');
...
try {
    const converterPath = path.join(__dirname, 'out/agents/UniversalConverterAgent.js');
    const validatorPath = path.join(__dirname, 'out/agents/UniversalValidatorAgent.js');
    
    if (fs.existsSync(converterPath) && fs.existsSync(validatorPath)) {
        console.log('✅ Universal Agents: Available');
        
        const { UniversalConverterAgent } = require('./out/agents/UniversalConverterAgent.js');
        const { UniversalValidatorAgent } = require('./out/agents/UniversalValidatorAgent.js');
        ...
        console.log(`   - Converter capabilities: ${converter.capabilities.join(', ')}`);
        console.log(`   - Validator capabilities: ${validator.capabilities.join(', ')}`);
    } else {
...
try {
    const hookPath = path.join(__dirname, 'out/hooks/hooks/UniversalValidationHook.js');
    if (fs.existsSync(hookPath)) {
        console.log('✅ Universal Validation Hook: Available');
        
        const { UniversalValidationHook } = require('./out/hooks/hooks/UniversalValidationHook.js');
...
try {
    const memoryPath = path.join(__dirname, 'out/memory/MemoryManager.production.js');
...
try {
    ...
    if (fs.existsSync(configPath)) {
        ...
        const configuredPairs = DEFAULT_CONFIG.languageConversion.supportedPairs.map(p => p.name);
...
console.log('Enhanced features have been successfully generalized for universal toolkit use.');
console.log('The system now supports multiple language conversion pairs beyond just C# to Rust.');
console.log('\n📝 Key Improvements:');
