...
import {
  claudePanel,
  debugMode,
  setDebugMode as coreSetDebugMode,
} from "../../core/state";
...
class Logger {
    ...
    try {
      ...
      if (workspaceFolder) {
        ...
        this.logFilePath = path.join(
          logDir,
          `autoclaude-${new Date().toISOString().split("T")[0]}.log`,
        );
        this.logToFile = true;
      }
    } catch (error) {
  ...
  log(
    ...
    // Console output - use console.log here to avoid recursion
    ...
    if (debugMode) {
      const webviewMessage = formatTerminalOutput(
        message,
        this.getLogTypeForWebview(level),
      );
      sendToWebviewTerminal(webviewMessage + "\n");
    }
  }

  private getLogTypeForWebview(
    level: LogLevel,
  ): "claude" | "debug" | "error" | "info" | "success" {
    switch (level) {
      case LogLevel.ERROR:
        return "error";
      case LogLevel.WARN:
...
export function formatTerminalOutput(
  text: string,
  type: "claude" | "debug" | "error" | "info" | "success",
): string {
  const now = new Date();
