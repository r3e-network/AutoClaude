...
const CLAUDE_PATTERNS = [
  ...
  /^.*generated\s+with\s+\[?claude\s+code\]?.*$/gim,
  /^.*co-authored-by:\s*claude.*$/gim,
  /^.*co-authored-by:\s*.*anthropic.*$/gim,
  /^.*🤖\s*(generated|created|written|assisted).*$/gim,
...
const REPLACEMENT_PATTERNS = [
  {
    pattern: /\b(implement|add|create|fix|update|refactor)\s+using\s+ai\b/gi,
    replacement: "$1",
  },
  {
    pattern: /\b(with\s+)?ai\s+(assistance|help|support)\b/gi,
    replacement: "",
  },
  {
    pattern: /\b(automatically\s+)?generated\s+code\b/gi,
  ...
  {
    pattern: /\b(llm|language model)\s+(generated|created)\b/gi,
    replacement: "implemented",
  },
];
