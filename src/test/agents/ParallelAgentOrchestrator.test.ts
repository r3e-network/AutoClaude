...
describe("ParallelAgentOrchestrator", () => {
  ...
  describe("Agent Management", () => {
    ...
    test("should enforce maximum agent limit", async () => {
      await expect(orchestrator.startAgents(100)).rejects.toThrow(
        "Cannot start more than 50 agents",
      );
    });
    ...
    test("should prevent starting agents when already running", async () => {
      ...
      await expect(orchestrator.startAgents(5)).rejects.toThrow(
  ...
  describe("Command Broadcasting", () => {
    ...
    test("should send command to all active agents", async () => {
      ...
      const sendCalls = execSpy.mock.calls.filter(
        (call: any[]) =>
          typeof call[0] === "string" &&
          call[0].includes("tmux send-keys") &&
          call[0].includes("/clear"),
      );
  ...
  describe("Error Handling", () => {
    ...
    test("should handle missing heartbeat files", async () => {
      ...
      await expect(
        (orchestrator as any).checkAgentHealth(),
