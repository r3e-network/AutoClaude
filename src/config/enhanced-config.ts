...
export class EnhancedConfigManager {
    ...
    constructor(private workspacePath: string) {
        this.configPath = path.join(workspacePath, '.autoclaude', 'enhanced-config.json');
    ...
    private async detectProjectEnvironment(): Promise<void> {
        try {
            ...
            const projectIndicators = [
                ...
                { files: ['package.json', 'tsconfig.json', 'src/index.tsx'], type: 'react-ts' },
                // Python indicators
    ...
    private async configureLanguageConversions(detectedLanguages: string[]): Promise<void> {
        ...
        if (detectedLanguages.includes('python') && detectedLanguages.includes('rust')) {
            debugLog('Python and Rust detected - Python to Rust conversion available');
        }
    }

    private async enableFeaturesForProjectType(projectType: string): Promise<void> {
        switch (projectType) {
            case 'neo-rs':
                this.config.neoRs.enabled = true;
                this.config.languageConversion.strictValidation = true;
                debugLog('Neo-rs project: Enabling strict validation and Neo-rs features');
                break;
            case 'react-ts':
            ...
            case 'csharp':
                ...
                if (fs.existsSync(path.join(this.workspacePath, 'Neo.VM')) || 
                    fs.existsSync(path.join(this.workspacePath, 'neo-cli'))) {
                    this.config.neoRs.enabled = true;
                    debugLog('Neo C# project detected - enabling Neo-rs conversion features');
                }
                break;
        }
    }
    ...
    validateConfig(): { isValid: boolean; errors: string[] } {
        ...
        if (this.config.agents.maxConcurrent < 1 || this.config.agents.maxConcurrent > 50) {
            errors.push('Max concurrent agents must be between 1 and 50');
        }

        // Validate memory configuration
        if (this.config.memory.maxSizeMB < 10 || this.config.memory.maxSizeMB > 1000) {
            errors.push('Memory max size must be between 10MB and 1000MB');
        }

        // Validate neo-rs configuration
        if (this.config.neoRs.maxFilesPerBatch < 1 || this.config.neoRs.maxFilesPerBatch > 100) {
    ...
    async detectNeoRsEnvironment(): Promise<boolean> {
        try {
            ...
            try {
                ...
                isNeoRsBranch = currentBranch === 'neo-rs' || currentBranch.includes('neo');
            } catch (error) {
            ...
            if (fs.existsSync(packageJsonPath)) {
                try {
                    ...
                    hasNeoDependencies = Object.keys(allDeps).some(dep => 
                        dep.includes('neo') || dep.includes('Neo')
                    );
                } catch (error) {
            ...
            const isNeoRsEnvironment = isNeoRsBranch || hasNeoRsFiles || hasNeoDependencies || hasNeoCSProject;
...
export class EnhancedConfigLoader {
    ...
    mergeConfigurations(base: EnhancedAutoClaudeConfig, override: Partial<EnhancedAutoClaudeConfig>): EnhancedAutoClaudeConfig {
    ...
    validateConfiguration(config: Partial<EnhancedAutoClaudeConfig>): {
        ...
        // Validate agent configuration
        if (config.agents) {
            if (config.agents.maxConcurrent && (config.agents.maxConcurrent < 1 || config.agents.maxConcurrent > 50)) {
...
export function getEnhancedConfig(workspacePath: string): EnhancedConfigManager {
