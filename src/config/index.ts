...
export function validateConfig(
  config: Partial<AutoClaudeConfig>,
  ...
  if (config.core) {
    if (
      ...
      !["error", "warn", "info", "debug"].includes(config.core.logLevel)
    ) {
  ...
  if (config.enhanced?.memory) {
    ...
    if (
      memory.maxSizeMB !== undefined &&
      (memory.maxSizeMB < 10 || memory.maxSizeMB > 10000)
    ) {
      warnings.push("Memory size should be between 10MB and 10GB");
      fixedValues["enhanced.memory.maxSizeMB"] = Math.max(
        10,
        Math.min(10000, memory.maxSizeMB),
      );
    }

    if (
      memory.patternConfidenceThreshold !== undefined &&
      (memory.patternConfidenceThreshold < 0 ||
        memory.patternConfidenceThreshold > 1)
    ) {
  ...
  if (config.enhanced?.agents) {
    ...
    if (
      agents.maxConcurrent !== undefined &&
      (agents.maxConcurrent < 1 || agents.maxConcurrent > 50)
    ) {
      warnings.push("Max concurrent agents should be between 1 and 50");
      fixedValues["enhanced.agents.maxConcurrent"] = Math.max(
        1,
        Math.min(50, agents.maxConcurrent),
      );
    }

    if (
      agents.coordinationStrategy &&
      !["sequential", "parallel", "adaptive"].includes(
        agents.coordinationStrategy,
      )
    ) {
      errors.push(
        `Invalid coordination strategy: ${agents.coordinationStrategy}`,
      );
    }
  }
  ...
  if (config.enhanced?.monitoring?.alertThresholds) {
    ...
    for (const { key, min, max } of thresholdValidation) {
      ...
      if (value !== undefined && (value < min || value > max)) {
        ...
        fixedValues[`enhanced.monitoring.alertThresholds.${key}`] = Math.max(
          min,
          Math.min(max, value),
        );
      }
...
export function mergeConfigs(
  base: AutoClaudeConfig,
  override: Partial<AutoClaudeConfig>,
): AutoClaudeConfig {
...
export function detectNeoRsEnvironment(workspaceRoot: string): boolean {
  try {
    ...
    const indicators = ["Cargo.toml", "neo-rs", "src/lib.rs", ".git"];
