...
export function validateConfig(
  ...
  if (
    config.developmentMode !== undefined &&
    typeof config.developmentMode !== "boolean"
  ) {
    addError(
      "developmentMode",
      config.developmentMode,
      "boolean",
      "Development mode must be true or false",
    );
  }
  ...
  if (config.queue) {
    ...
    if (q.maxMessageSize !== undefined) {
      if (
        typeof q.maxMessageSize !== "number" ||
        q.maxMessageSize < 1000 ||
        q.maxMessageSize > 1000000
      ) {
        addError(
          "queue.maxMessageSize",
          q.maxMessageSize,
          "number (1000-1000000)",
          "Must be a number between 1KB and 1MB",
        );
      }
    }
    ...
    if (q.retentionHours !== undefined) {
      if (
        typeof q.retentionHours !== "number" ||
        q.retentionHours < 1 ||
        q.retentionHours > 168
      ) {
        addError(
          "queue.retentionHours",
          q.retentionHours,
          "number (1-168)",
          "Must be a number between 1 and 168 hours (1 week)",
        );
      }
    }
  }
  ...
  if (config.session) {
    ...
    if (s.autoStart !== undefined && typeof s.autoStart !== "boolean") {
      addError(
        ...
        s.autoStart,
        "boolean",
        "Auto start must be true or false",
      );
    }

    if (
      s.skipPermissions !== undefined &&
      typeof s.skipPermissions !== "boolean"
    ) {
      addError(
        "session.skipPermissions",
        s.skipPermissions,
        "boolean",
        "Skip permissions must be true or false",
      );
    }
    ...
    if (s.scheduledStartTime !== undefined) {
      if (typeof s.scheduledStartTime !== "string") {
        addError(
          ...
          "string",
          "Scheduled start time must be a string",
        );
      } else if (
        s.scheduledStartTime !== "" &&
        !/^([01]?[0-9]|2[0-3]):[0-5][0-9]$/.test(s.scheduledStartTime)
      ) {
        addError(
          "session.scheduledStartTime",
          s.scheduledStartTime,
          "HH:MM format",
          'Scheduled start time must be in HH:MM format (e.g., "09:30") or empty string to disable',
        );
      }
    }
    ...
    if (
      s.autoStart === true &&
      s.scheduledStartTime !== undefined &&
      s.scheduledStartTime !== ""
    ) {
      addError(
        "session.autoStart + session.scheduledStartTime",
        "both enabled",
        "only one enabled",
        "Cannot use both autoStart and scheduledStartTime - choose one or the other",
      );
    }
  }
  ...
  if (config.sleepPrevention) {
    ...
    if (sp.enabled !== undefined && typeof sp.enabled !== "boolean") {
      addError(
        "sleepPrevention.enabled",
        sp.enabled,
        "boolean",
        "Sleep prevention enabled must be true or false",
      );
    }
  ...
  if (config.history) {
    ...
    if (h.autoSave !== undefined && typeof h.autoSave !== "boolean") {
      addError(
        ...
        h.autoSave,
        "boolean",
        "Auto save must be true or false",
      );
    }

    if (
      h.persistPendingQueue !== undefined &&
      typeof h.persistPendingQueue !== "boolean"
    ) {
      addError(
        "history.persistPendingQueue",
        h.persistPendingQueue,
        "boolean",
        "Persist pending queue must be true or false",
      );
    }
    ...
    if (h.showInUI !== undefined && typeof h.showInUI !== "boolean") {
      addError(
        ...
        h.showInUI,
        "boolean",
        "Show in UI must be true or false",
      );
    }
  }
  ...
  if (config.security) {
    ...
    if (
      sec.allowDangerousXssbypass !== undefined &&
      typeof sec.allowDangerousXssbypass !== "boolean"
    ) {
      addError(
        "security.allowDangerousXssbypass",
        sec.allowDangerousXssbypass,
        "boolean",
        "Allow dangerous XSS bypass must be true or false",
      );
    }
  }
...
export function getValidatedConfig(): ClaudeAutopilotConfig {
  try {
    ...
    const config: ClaudeAutopilotConfig = {
      developmentMode: workspaceConfig.get(
        "developmentMode",
        DEFAULT_CONFIG.developmentMode,
      ),

      queue: {
        maxSize: workspaceConfig.get(
          "queue.maxSize",
          DEFAULT_CONFIG.queue.maxSize,
        ),
        maxMessageSize: workspaceConfig.get(
          "queue.maxMessageSize",
          DEFAULT_CONFIG.queue.maxMessageSize,
        ),
        maxOutputSize: workspaceConfig.get(
          "queue.maxOutputSize",
          DEFAULT_CONFIG.queue.maxOutputSize,
        ),
        maxErrorSize: workspaceConfig.get(
          "queue.maxErrorSize",
          DEFAULT_CONFIG.queue.maxErrorSize,
        ),
        cleanupThreshold: workspaceConfig.get(
          "queue.cleanupThreshold",
          DEFAULT_CONFIG.queue.cleanupThreshold,
        ),
        retentionHours: workspaceConfig.get(
          "queue.retentionHours",
          DEFAULT_CONFIG.queue.retentionHours,
        ),
      },

      session: {
        autoStart: workspaceConfig.get(
          "session.autoStart",
          DEFAULT_CONFIG.session.autoStart,
        ),
        skipPermissions: workspaceConfig.get(
          "session.skipPermissions",
          DEFAULT_CONFIG.session.skipPermissions,
        ),
        scheduledStartTime: workspaceConfig.get(
          "session.scheduledStartTime",
          DEFAULT_CONFIG.session.scheduledStartTime,
        ),
        autoResumeUnfinishedTasks: workspaceConfig.get(
          "session.autoResumeUnfinishedTasks",
          DEFAULT_CONFIG.session.autoResumeUnfinishedTasks,
        ),
      },

      sleepPrevention: {
        enabled: workspaceConfig.get(
          "sleepPrevention.enabled",
          DEFAULT_CONFIG.sleepPrevention.enabled,
        ),
        method: workspaceConfig.get(
          "sleepPrevention.method",
          DEFAULT_CONFIG.sleepPrevention.method,
        ),
      },

      history: {
        maxRuns: workspaceConfig.get(
          "history.maxRuns",
          DEFAULT_CONFIG.history.maxRuns,
        ),
        autoSave: workspaceConfig.get(
          "history.autoSave",
          DEFAULT_CONFIG.history.autoSave,
        ),
        persistPendingQueue: workspaceConfig.get(
          "history.persistPendingQueue",
          DEFAULT_CONFIG.history.persistPendingQueue,
        ),
        showInUI: workspaceConfig.get(
          "history.showInUI",
          DEFAULT_CONFIG.history.showInUI,
        ),
      },

      security: {
        allowDangerousXssbypass: workspaceConfig.get(
          "security.allowDangerousXssbypass",
          DEFAULT_CONFIG.security.allowDangerousXssbypass,
        ),
      },
    };
    ...
    if (errors.length > 0) {
      ...
      errors.forEach((error) => {
        debugLog(
          `  - ${error.path}: ${error.message} (got: ${error.value}, expected: ${error.expected})`,
        );
        ErrorManager.logError(
          CommonErrors.INVALID_CONFIGURATION(
            error.path,
            error.value,
            error.expected,
          ),
...
function getDefaultsForInvalidConfig(
  config: ClaudeAutopilotConfig,
  errors: ConfigValidationError[],
): ClaudeAutopilotConfig {
  ...
  Promise.all(resetPromises)
    .then(() => {
      vscode.window.showInformationMessage(
        "AutoClaude configuration reset to defaults",
      );
...
export function showConfigValidationStatus(): void {
  ...
  if (errors.length === 0) {
    vscode.window.showInformationMessage(
      "✅ AutoClaude configuration is valid",
    );
  } else {
    const errorSummary =
    ...
    vscode.window
      .showWarningMessage(errorSummary, "Open Settings", "Reset to Defaults")
      .then((selection) => {
...
export function showConfigValidationDetails(
  errors: ConfigValidationError[],
): void {
  const details = [
...
export function watchConfigChanges(
  callback: (config: ClaudeAutopilotConfig) => void,
): vscode.Disposable {
