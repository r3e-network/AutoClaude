...
export class ResilienceManager {
  ...
  async retryWithBackoff<T>(
    ...
    for (let attempt = 1; attempt <= config.retryAttempts; attempt++) {
      try {
        ...
        debugLog(
          `Retry attempt ${attempt}/${config.retryAttempts} for ${serviceName} failed: ${lastError.message}`,
        );
    ...
    ErrorManager.logError(lastError!, {
      serviceName,
  ...
  async executeWithDegradation<T, F>(
    ...
    try {
      ...
      errorLog(
        `Primary operation failed for ${serviceName}, falling back to degraded mode`,
        {
      ...
      this.markServiceDegraded(
        serviceName,
        degradationMessage || "Primary operation failed, using fallback",
      );
  ...
  async executeWithBulkhead<T>(
    ...
    try {
      ...
      const duration = Date.now() - startTime;
      errorLog(
        `Bulkhead ${bulkheadName} operation failed after ${duration}ms`,
        {
          error: error instanceof Error ? error.message : String(error),
        },
      );
  ...
  private getOrCreateCircuitBreaker(
    serviceName: string,
    options?: { timeout?: number; errorThreshold?: number; resetTimeout?: number },
  ): CircuitBreaker {
  ...
  showHealthDashboard(): void {
    const healthData = Array.from(this.serviceHealth.entries()).map(
      ([name, health]) => {
        const statusIcon = {
...
export async function executeWithFallback<T, F>(
  ...
  return resilience.executeWithDegradation(
    primary,
    fallback,
    serviceName,
    degradationMessage,
