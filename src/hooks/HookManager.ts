...
class AutoFormatHook implements Hook {
    ...
    async execute(context: HookContext): Promise<HookResult> {
        ...
        try {
            ...
            return {
                success: false,
                error: `Auto formatting failed: ${error.message}`,
                executionTime: Date.now() - startTime
            };
        }
    }
    ...
    private async formatRust(context: HookContext): Promise<HookResult> {
        try {
            ...
            return {
                success: false,
                error: `rustfmt not available or failed: ${error.message}`,
...
class PatternLearningHook implements Hook {
    ...
    async execute(context: HookContext): Promise<HookResult> {
        ...
        try {
            ...
            return {
                success: false,
                error: `Pattern learning failed: ${error.message}`,
                executionTime: Date.now() - startTime
            };
        }
    }

    private extractPatterns(csharpCode: string, rustCode: string): Array<{
        csharp: string;
        rust: string;
        ...
        for (const typePattern of typePatterns) {
            if (csharpCode.includes(typePattern.csharp) && rustCode.includes(typePattern.rust)) {
                patterns.push({
...
class NeoRsValidationHook implements Hook {
    ...
    async execute(context: HookContext): Promise<HookResult> {
        ...
        try {
            ...
            return {
                success: false,
                error: `Neo-rs validation failed: ${error.message}`,
                executionTime: Date.now() - startTime
            };
        }
    }
    ...
    private async validateNeoRsConversion(context: HookContext): Promise<{
        ...
        for (const neoType of neoTypes) {
            if (context.originalContent?.includes(neoType) && !content.includes(neoType)) {
        ...
        if (context.originalContent?.includes('try') && !content.includes('Result<')) {
    ...
    private calculateApiCompatibility(csharpCode: string, rustCode: string): number {
        // Simple API compatibility calculation
        const csharpMethods = (csharpCode.match(/public\s+\w+\s+\w+\s*\(/g) || []).length;
...
export class HookManager {
    ...
    private async loadCustomHooks(): Promise<void> {
        ...
        const hooksPath = path.resolve(this.workspacePath, config.hooks.customHooksPath);
    ...
    async executeHooks(
        ...
        for (const hook of hooks) {
            ...
            try {
                ...
                if (hook.blocking) {
                    debugLog(`Blocking hook ${hook.name} threw error, aborting execution`);
                    return { success: false, results };
                }
            }
        }
        ...
        const success = results.every(r => r.success) || 
                       results.some(r => r.success && !hooks.find(h => h.blocking && !r.success));
    ...
    async registerHook(hook: Hook, operation?: string): Promise<void> {
        if (!operation) {
            ...
            const operations = ['pre-conversion', 'post-conversion', 'pre-validation', 'post-validation'];
        ...
        debugLog(`Hook ${hook.name} registered for operation: ${operation || 'multiple'}`);
    }
