        ...
        lines.forEach((line, index) => {
            ...
            if (trimmed.includes('public class') && !trimmed.includes('{')) {
                ...
                if (!nextLine || !nextLine.trim().startsWith('{')) {
                    warnings.push(`Line ${lineNum}: Class declaration should be followed by opening brace`);
                }
            }
        });
    ...
    private validateRust(code: string): {
        ...
        lines.forEach((line, index) => {
            ...
            if (trimmed.includes('unsafe')) {
                warnings.push(`Line ${lineNum}: Unsafe block detected, ensure safety requirements are met`);
            }
        });
