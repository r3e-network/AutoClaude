...
export class PatternLearningHook implements Hook {
    ...
    public readonly description: string = 'Learns patterns from successful conversions';
    ...
    async execute(context: HookContext): Promise<HookResult> {
        ...
        try {
            ...
            return {
                ...
                modified: false,
                warnings: [`Pattern learning failed: ${error instanceof Error ? error.message : String(error)}`],
                duration: Date.now() - startTime
            };
        }
    }

    private async learnFromConversion(input: any): Promise<{
        ...
        for (const pattern of patterns) {
            try {
                ...
                const existingPatterns = await memoryManager.getPatterns(pattern.category, 0.1);
                const existing = existingPatterns.find((p: any) => 
                    p.input_pattern === pattern.input && p.output_pattern === pattern.output
                );
                
                if (existing) {
                    // Update existing pattern with higher confidence
                    const newConfidence = Math.max(existing.confidence, pattern.confidence);
                    await memoryManager.updatePatternSuccess(existing.id, true, newConfidence);
    ...
    private async learnFromValidation(input: any): Promise<{
        ...
        try {
            ...
            if (success && validationResult.compatibilityScore > 80) {
                ...
                const patterns = this.extractValidationPatterns(originalCode, convertedCode, validationResult);
    ...
    private extractPatterns(originalCode: string, convertedCode: string): Array<{
        input: string;
        output: string;
        category: string;
        confidence: number;
        ...
        for (let i = 0; i < maxLines; i++) {
            ...
            if (originalLine.length > 0 && convertedLine.length > 0 && originalLine !== convertedLine) {
                // Detect pattern type
                const category = this.detectPatternCategory(originalLine, convertedLine);
                const confidence = this.calculatePatternConfidence(originalLine, convertedLine, category);
    ...
    private extractValidationPatterns(
        ...
        if (validationResult.compatibilityScore > 90) {
            const neoPatterns = this.extractNeoSpecificPatterns(originalCode, convertedCode);
    ...
    private extractBlockPatterns(originalCode: string, convertedCode: string): Array<{
        input: string;
        output: string;
        category: string;
        confidence: number;
        ...
        while ((methodMatch = methodRegex.exec(originalCode)) !== null) {
            ...
            while ((functionMatch = functionRegex.exec(convertedCode)) !== null) {
                if (methodMatch[2] === functionMatch[1]) { // Same method name
                    patterns.push({
                        input: methodMatch[0],
    ...
    private extractTypePatterns(originalCode: string, convertedCode: string): Array<{
        input: string;
        output: string;
        category: string;
        confidence: number;
    ...
    private extractNeoSpecificPatterns(originalCode: string, convertedCode: string): Array<{
        input: string;
        output: string;
        category: string;
        confidence: number;
        ...
        const neoTypes = [
            'UInt160', 'UInt256', 'ECPoint', 'StackItem', 'InteropService',
            'ApplicationEngine', 'ExecutionEngine', 'StorageContext', 'ContractState'
        ];
        ...
        for (const neoType of neoTypes) {
            if (originalCode.includes(neoType)) {
                ...
                for (const line of lines) {
                    if (line.includes('U160') || line.includes('U256') || line.includes('PublicKey')) {
                        patterns.push({
                            input: neoType,
                            output: line.trim(),
    ...
    private detectPatternCategory(originalLine: string, convertedLine: string): string {
        if (originalLine.includes('public class') && convertedLine.includes('pub struct')) {
        ...
        if (originalLine.includes('string') || originalLine.includes('int') || originalLine.includes('bool')) {
    ...
    private calculatePatternConfidence(originalLine: string, convertedLine: string, category: string): number {
        ...
        const wordRatio = Math.min(originalWords, convertedWords) / Math.max(originalWords, convertedWords);
