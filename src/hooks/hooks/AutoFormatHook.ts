...
export class AutoFormatHook implements Hook {
    ...
    public readonly description: string = 'Automatically formats code with language-specific formatters';
    ...
    async execute(context: HookContext): Promise<HookResult> {
        ...
        try {
            ...
            return {
                ...
                modified: false,
                warnings: [`Formatting failed: ${error instanceof Error ? error.message : String(error)}`],
                duration: Date.now() - startTime
            };
        }
    }
    ...
    private async formatRust(code: string, filePath?: string): Promise<{
        formattedCode: string;
        modified: boolean;
        warnings?: string[];
    }> {
        try {
            ...
            return {
                formattedCode: this.basicRustFormat(code),
                modified: true,
                warnings: [`rustfmt failed: ${error instanceof Error ? error.message : String(error)}, using basic formatting`]
            };
        }
    }
    ...
    private async formatCSharp(code: string): Promise<{
        ...
        try {
            ...
            return {
                formattedCode: code,
                modified: false,
                warnings: [`C# formatting failed: ${error instanceof Error ? error.message : String(error)}`]
            };
        }
    }
    ...
    private basicCSharpFormat(code: string): string {
        ...
        for (const line of lines) {
            ...
            if (trimmed.includes('namespace') || trimmed.includes('class') || trimmed.includes('interface')) {
    ...
    private formatCSharpPatterns(line: string): string {
        ...
        formatted = formatted.replace(/\b(if|else|for|while|switch|catch|using)\(/g, '$1 (');
