...
export class UniversalValidationHook implements Hook {
  ...
  public readonly description: string =
    "Validates language conversion compatibility";
  ...
  private memoryManager: ValidationMemoryManager | null = null;
  private config: ValidationEnhancedConfig | null = null;

  constructor() {
    // Initialize when needed
  }

  private async getMemoryManager(workspacePath?: string): Promise<ValidationMemoryManager | null> {
  ...
  private async getConfig(workspacePath?: string): Promise<ValidationEnhancedConfig | null> {
    ...
    try {
      ...
      switch (operation) {
        case "post-conversion":
          validationResult = await this.validateConversion(
            input,
            enhancedConfig,
          );
          break;
        case "pre-validation":
          validationResult = await this.validateBeforeProcessing(
            input,
            enhancedConfig,
          );
          break;
        case "post-validation":
          validationResult = await this.validateFinalResult(
            input,
            enhancedConfig,
          );
          break;
  ...
  private async validateConversion(
    ...
    const languagePair = await this.detectLanguagePair(
      originalCode,
      convertedCode,
      metadata,
    );
    ...
    if (!languagePair) {
      warnings.push(
        "Could not detect specific language pair, using generic validation",
      );
    }
  ...
  private async validateLanguagePair(
    ...
    const errors: string[] = [];
    const warnings: string[] = [];
    const results: Record<string, TypeValidationResult | StructureValidationResult> = {};

    if (!languagePair) {
      // Generic validation
      results.structureValidation = await this.validateStructure(
        originalCode,
        convertedCode,
      );
      ...
      const conversionPair = config.languageConversion.supportedPairs.find(
        (pair: LanguageConversion) =>
          pair.from === languagePair.from && pair.to === languagePair.to,
      );
    ...
    for (const [sourceType, targetType] of Object.entries(typeMappings)) {
      if (originalCode.includes(sourceType)) {
        if (!convertedCode.includes(targetType)) {
          errors.push(
            `Type '${sourceType}' should be converted to '${targetType}'`,
          );
        } else {
          // Check usage count
          const sourceCount = (
            originalCode.match(new RegExp(sourceType, "g")) || []
          ).length;
          const targetCount = (
            convertedCode.match(new RegExp(targetType, "g")) || []
          ).length;

          if (Math.abs(sourceCount - targetCount) > 2) {
            warnings.push(
              `Usage count mismatch for ${sourceType} -> ${targetType}: ${sourceCount} vs ${targetCount}`,
            );
          }
        }
      }
  ...
  private async validateStructure(
    ...
    const lineRatio =
      Math.min(originalLines, convertedLines) /
      Math.max(originalLines, convertedLines);
    if (lineRatio < 0.5) {
      warnings.push(
        `Significant line count difference: ${originalLines} vs ${convertedLines}`,
      );
      score -= 10;
    }
    ...
    for (const marker of structureMarkers) {
      const originalCount = (originalCode.match(new RegExp(marker, "gi")) || [])
        .length;
      const convertedCount = (
        convertedCode.match(new RegExp(marker, "gi")) || []
      ).length;

      if (originalCount > 0 && convertedCount === 0) {
        warnings.push(
          `Structure element '${marker}' appears to be missing in conversion`,
        );
        score -= 5;
      }
    }
  ...
  private async performSpecialValidation(
    ...
    if (languagePair.from === "csharp" && languagePair.to === "rust") {
      ...
      if (
        originalCode.includes("ApplicationEngine") &&
        !convertedCode.includes("ApplicationEngine")
      ) {
        errors.push(
      ...
      if (
        originalCode.includes("throw new") &&
        !convertedCode.includes("Result<")
      ) {
        warnings.push(
          "Exception handling should use Result<T, E> pattern in Rust",
        );
      }
    }
  ...
  private async validateBeforeProcessing(
    input: ValidationInput,
    config: ValidationConfig,
  ): Promise<{
    isValid: boolean;
    ...
    if (language) {
      ...
      const supportedConversions =
        config.languageConversion.supportedPairs.filter(
          (pair: LanguageConversion) => pair.from === language,
        );

      if (supportedConversions.length > 0) {
        const targets = supportedConversions.map((p: LanguageConversion) => p.to).join(", ");
        warnings.push(
          `${language} detected - conversions available to: ${targets}`,
        );
      }
    }
  ...
  private async validateFinalResult(
    input: ValidationInput,
    config: ValidationConfig,
  ): Promise<{
    isValid: boolean;
  ...
  private async validateStructuralIntegrity(
    ...
    for (const element of structureElements) {
      ...
      const convertedCount = (convertedCode.match(element.pattern) || [])
        .length;

      if (originalCount > 0 && Math.abs(originalCount - convertedCount) > 2) {
        issues.push(
          `${element.name} count mismatch: ${originalCount} vs ${convertedCount}`,
        );
        structureScore -= 15;
      }
    }
