          ...
          (match, funcName, params) => {
            ...
            const typedParams = params
              ...
              .map((param: string) => {
                ...
                if (trimmed.match(/^(id|count|index|length|size)$/i))
                  return `${trimmed}: number`;
                if (trimmed.match(/^(is|has|should|can|will)[\w]/i))
                  return `${trimmed}: boolean`;
                if (trimmed.match(/^(name|text|message|path|url)$/i))
                  return `${trimmed}: string`;
                if (trimmed.match(/^(data|options|config|props)$/i))
                  return `${trimmed}: Record<string, any>`;
                if (trimmed.match(/^(callback|handler|fn)$/i))
                  return `${trimmed}: Function`;
                if (trimmed.match(/^(error|err|e)$/i))
        ...
        for (const match of importMatches) {
          ...
          if (foundFiles.length > 0) {
            const relativePath = path
              .relative(testDir, foundFiles[0].fsPath)
              .replace(/\\/g, "/");
