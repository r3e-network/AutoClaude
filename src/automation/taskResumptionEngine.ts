...
export class TaskResumptionEngine {
  ...
  async resumeTask(taskId: string): Promise<string> {
    ...
    this.taskManager.addTaskContext(taskId, {
      outputs: [
        `Resuming task after interruption. Strategy: ${analysis.strategy}`,
      ],
    });
   ...
   */
  private async analyzeTaskState(
    task: PersistedTask,
...
`);
   ...
   */
  private async getNextStepsForContinuation(
    task: PersistedTask,
  ): Promise<string[]> {
    const steps: string[] = [];
    ...
    if (task.subtasks && task.subtasks.length > 0) {
      const incompleteSubtasks = this.taskDecomposer.getIncompleteSubtasks(
        task.id,
      );
  ...
  private async getRetrySteps(task: PersistedTask): Promise<string[]> {
    ...
    if (lastError?.includes("ENOENT") || lastError?.includes("not found")) {
      ...
      lastError?.includes("dependency") ||
      lastError?.includes("module")
    ) {
  ...
  private async checkTaskHealth(
    ...
    } else if (timeSinceLastCheck > 5 * 60 * 1000) {
      // 5 minutes inactive
   ...
   */
  private async handleCriticalTask(
    task: PersistedTask,
    monitor: TaskMonitor,
  ): Promise<void> {
   ...
   */
  private async handleWarningTask(
    task: PersistedTask,
    monitor: TaskMonitor,
  ): Promise<void> {
...
`,
    ...
    if (task.subtasks && task.subtasks.length > 0) {
      const incompleteSubtasks =
        this.taskDecomposer.getIncompleteSubtasks(taskId);
      if (incompleteSubtasks.length > 0) {
  ...
  cleanupCompletedTasks(): void {
    const completedTasks = Array.from(this.activeMonitors.keys()).filter(
      (taskId) => {
        const task = this.taskManager.getTask(taskId);
