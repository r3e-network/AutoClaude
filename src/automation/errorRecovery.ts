...
export class ErrorRecoverySystem {
  ...
  private errorPatterns: ErrorPattern[] = [
    // TypeScript errors
    {
      ...
      solution:
        "Check if the module exists or needs to be installed. Verify the import path.",
      priority: 1,
    },
    {
    ...
    },
    {
      ...
      type: ErrorType.TypeScript,
      solution:
        "Fix the type mismatch by updating the argument or parameter type.",
      priority: 2,
    },
    ...
    // Test failures
    {
      ...
      solution:
        "Update the test expectation or fix the implementation to match.",
      priority: 3,
    },
    {
      ...
      type: ErrorType.TestFailure,
      solution:
        "Check test setup and dependencies. Ensure all imports are correct.",
      priority: 1,
    },
    ...
    try {
      ...
      if (success) {
        ...
        vscode.window.showWarningMessage(
          `⚠️ Recovery failed: ${strategy.description}`,
        );
      }
  ...
  private findErrorPattern(error: string): ErrorPattern | null {
    // Sort by priority
    const sorted = [...this.errorPatterns].sort(
   ...
   */
  private createImportErrorStrategy(
    error: string,
    pattern: ErrorPattern,
  ): RecoveryStrategy {
   ...
   */
  private createTypeScriptErrorStrategy(
    error: string,
    pattern: ErrorPattern,
  ): RecoveryStrategy {
    return {
   ...
   */
  private createTestFailureStrategy(
    error: string,
    pattern: ErrorPattern,
  ): RecoveryStrategy {
    return {
   ...
   */
  private createBuildErrorStrategy(
    error: string,
    pattern: ErrorPattern,
  ): RecoveryStrategy {
    return {
   ...
   */
  private createLintErrorStrategy(
    error: string,
    pattern: ErrorPattern,
  ): RecoveryStrategy {
    return {
      ...
      action: async () => {
        ...
        await new Promise((resolve) =>
          setTimeout(resolve, this.SHORT_DELAY_MS),
        );
  ...
  async attemptRecovery(error: string): Promise<RecoveryStrategy | null> {
    // This is an alias for analyzeError for backward compatibility
