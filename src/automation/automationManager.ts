...
export class AutomationManager {
  ...
  async initialize() {
    try {
      ...
      if (this.environmentInfo) {
        debugLog(
          `Environment detected: Neo-rs=${this.environmentInfo.isNeoRs}, Branch=${this.environmentInfo.branch}, Neo Branch=${this.environmentInfo.isNeoRsBranch}`,
        );

        if (this.environmentInfo.isNeoRs) {
          vscode.window.showInformationMessage(
            "AutoClaude: Neo-rs environment detected. Enhanced features enabled.",
          );
        }
      }
    } catch (error) {
      ...
      vscode.window.showErrorMessage(
        `Automation initialization failed: ${error instanceof Error ? error.message : String(error)}`,
      );
  ...
  private async detectEnvironment(): Promise<void> {
    try {
      ...
      debugLog(
        `Environment detection complete: Neo-rs=${isNeoRs}, Branch=${branch}, Neo Branch=${isNeoRsBranchDetected}`,
      );
    } catch (error) {
  ...
  async processMessage(message: string, currentFile?: string): Promise<string> {
    ...
    try {
      ...
      const automatedPrompt = this.addEnvironmentAwareInstructions(
        enhancedPrompt,
        message,
      );
  ...
  async runValidation(): Promise<void> {
    try {
      ...
      if (!allPassed) {
        ...
        const failedScripts = Array.from(results.entries()).filter(
   ...
   */
  private addEnvironmentAwareInstructions(
    prompt: string,
...
8. Consider security implications of changes`;
    ...
    if (
      this.environmentInfo &&
      (this.environmentInfo.isNeoRs || this.environmentInfo.isNeoRsBranch)
    ) {
      instructions += `
...
17. Ensure proper handling of Neo cryptographic operations`;
      ...
      const isConversionTask =
        /convert|port|migrate|translate.*c#.*rust|rust.*c#/i.test(
          originalMessage,
        );
      if (isConversionTask) {
        instructions += `
...
=== End Automation Instructions ===`;
  ...
  getStatistics() {
    return {
      ...
      activeWorkflows: this._commandOrchestrator
        ? this._commandOrchestrator.getActiveWorkflows().length
        : 0,
