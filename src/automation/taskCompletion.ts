...
export class TaskCompletionEngine {
      ...
      async (progress, token) => {
        ...
        if (suggestions.length === 0) {
          vscode.window.showInformationMessage(
            "✅ No immediate tasks detected. Your project looks good!",
          );
          return;
        }
        ...
        progress.report({
          increment: 20,
          message: "Identifying automatic tasks...",
        });
        const autoTasks = suggestions.filter(
          (s) => s.confidence > 0.8 && s.category === "fix",
        );
        const manualTasks = suggestions.filter(
          (s) => s.confidence <= 0.8 || s.category !== "fix",
        );
        ...
        if (autoTasks.length > 0) {
          progress.report({
            increment: 30,
            message: "Executing automatic fixes...",
          });
  ...
  private async assessProjectHealth(): Promise<
    "excellent" | "good" | "needs-work" | "critical"
  > {
    if (!this.scriptRunner) return "good";

    const results = await this.scriptRunner.runChecks(false);
    const failedCount = Array.from(results.results.values()).filter(
      (r) => !r.passed,
    ).length;
  ...
  private generateScriptBasedSuggestions(
    ...
    switch (health) {
      case "critical":
        suggestions.push({
          ...
          description:
            "Your project has critical issues that need immediate attention",
          category: "fix",
          confidence: 0.95,
      ...
      case "excellent":
        suggestions.push({
          ...
          description:
            "Get detailed insights and suggestions for your excellent code",
          category: "analyze",
          confidence: 0.6,
          estimatedTime: "5-10 minutes",
          priority: 30,
    ...
    if (context.activeEditor) {
      ...
      if (fileName.endsWith(".test.ts") || fileName.endsWith(".spec.ts")) {
        suggestions.push({
          ...
          description:
            "Enhance your test file with better coverage and practices",
          category: "improve",
          confidence: 0.7,
          estimatedTime: "3-10 minutes",
          priority: 60,
      ...
      if (fileName.endsWith(".md") || fileName.endsWith(".README")) {
        suggestions.push({
          ...
          description:
            "Enhance your documentation with better structure and content",
          category: "improve",
          confidence: 0.6,
