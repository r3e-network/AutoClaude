...
export class IntelligentTaskAnalyzer {
    ...
    private agentCapabilities = {
        'architect-agent': ['system-design', 'api-design', 'database-design', 'architecture-planning', 'ddd-practices'],
        'coder-agent': ['implementation', 'refactoring', 'bug-fixing', 'feature-development', 'tdd-practices'],
        'tester-agent': ['unit-testing', 'integration-testing', 'test-generation', 'coverage-analysis', 'tdd-practices', 'tdd-validation'],
        'researcher-agent': ['code-analysis', 'dependency-analysis', 'best-practices', 'pattern-discovery'],
        'security-agent': ['vulnerability-scanning', 'security-audit', 'threat-modeling', 'compliance-check'],
        'documentation-agent': ['api-docs', 'user-guides', 'technical-specs', 'readme-generation', 'ddd-practices', 'ddd-validation', 'technical-writing'],
        'optimization-agent': ['performance-tuning', 'memory-optimization', 'build-optimization', 'algorithm-improvement'],
        'production-readiness': ['production-check', 'deployment-ready', 'quality-assurance', 'tdd-validation', 'ddd-validation'],
        'tdd-enforcement': ['tdd-practices', 'test-first-validation', 'test-execution', 'tdd-validation'],
        'ddd-enforcement': ['ddd-practices', 'documentation-first', 'technical-writing', 'ddd-validation'],
    ...
    async analyzeTask(task: string | HiveMindTask): Promise<TaskAnalysisResult> {
        ...
        log.info('Analyzing task for intelligent decomposition', { task: taskDescription });
        ...
        try {
            ...
            const requiredCapabilities = this.determineRequiredCapabilities(intent, context);
            ...
            const executionPlan = this.createExecutionPlan(subtasks, assignments, toolRequirements);
            
            // Step 9: Learn from this analysis
            await this.learnFromAnalysis(taskDescription, intent, subtasks, executionPlan);
     ...
     */
    private async analyzeTaskIntent(taskDescription: string): Promise<TaskIntent> {
        ...
        return {
            ...
            requiresTesting: targetType !== 'test' && (actionKeywords.create.some(k => words.includes(k)) || actionKeywords.fix.some(k => words.includes(k))),
            requiresDocumentation: targetType !== 'documentation' && actionKeywords.create.some(k => words.includes(k))
        };
    }
     ...
     */
    private async findSimilarTasks(taskDescription: string, intent: TaskIntent): Promise<SimilarTask[]> {
        try {
            // Search memory for similar task patterns
            const searchKey = `${intent.primaryAction}_${intent.targetType}`;
            const similarPatterns = await this.memoryManager.searchPatterns?.(searchKey) || [];
            
            // Also search by task description similarity
            const textSimilar = await this.memoryManager.searchSimilarTasks?.(taskDescription) || [];
            ...
            return allSimilar
                .map(task => ({
                    task,
                    similarity: this.calculateSimilarity(taskDescription, task.description || ''),
     ...
     */
    private async gatherTaskContext(taskDescription: string, intent: TaskIntent): Promise<TaskContext> {
     ...
     */
    private determineRequiredCapabilities(intent: TaskIntent, context: TaskContext): string[] {
    ...
    private async decomposeIntoSubtasks(
        ...
        if (similarTasks.length > 0 && similarTasks[0].similarity > 0.7) {
            ...
            if (bestMatch.successful && bestMatch.subtasks.length > 0) {
                ...
                return this.adaptSubtasksFromPattern(bestMatch.subtasks, intent, context);
            }
        }
        ...
        if (intent.requiresDocumentation || intent.requiresNewCode) {
            subtasks.push({
                ...
                type: 'documentation',
                requiredCapabilities: ['documentation', 'api-docs', 'technical-writing'],
                dependencies: [],
        ...
        if (intent.requiresTesting || intent.requiresNewCode || intent.primaryAction === 'fix') {
            const docDeps = subtasks.filter(s => s.type === 'documentation').map(s => s.id);
            subtasks.push({
                id: `subtask-${Date.now()}-${order++}`,
                description: `Write comprehensive failing tests for ${intent.targetType}`,
                type: 'testing',
                requiredCapabilities: ['test-generation', 'coverage-analysis', 'tdd-practices'],
        ...
        if (intent.primaryAction === 'create' || intent.primaryAction === 'fix' || intent.primaryAction === 'improve') {
            // Implementation MUST depend on both documentation and tests (TDD/DDD enforcement)
            const docTestDeps = subtasks.filter(s => 
                s.type === 'documentation' || s.type === 'testing'
            ).map(s => s.id);
            ...
            if (intent.entities.length > 1) {
                for (const entity of intent.entities) {
                    subtasks.push({
                        ...
                        type: 'implementation',
                        requiredCapabilities: ['implementation', 'refactoring', 'tdd-practices'],
                ...
                subtasks.push({
                    ...
                    type: 'implementation',
                    requiredCapabilities: ['implementation', 'bug-fixing', 'tdd-practices'],
                    dependencies: docTestDeps, // CRITICAL: Must have docs and tests first
        ...
        if (intent.requiresTesting || intent.requiresNewCode) {
            ...
            const testValidationDeps = implTasks.length > 0 ? implTasks.map(s => s.id) : [];
            ...
            if (testValidationDeps.length > 0) {
                subtasks.push({
                    ...
                    requiredCapabilities: ['test-execution', 'coverage-analysis', 'tdd-validation'],
        ...
        if (intent.targetType === 'security' || intent.targetType === 'api' || requiredCapabilities.includes('security-audit')) {
            subtasks.push({
                id: `subtask-${Date.now()}-${order++}`,
                description: `Perform security audit and vulnerability scan`,
                type: 'security',
                requiredCapabilities: ['security-audit', 'vulnerability-scanning'],
                dependencies: subtasks.filter(s => s.type === 'implementation').map(s => s.id),
        ...
        if (intent.targetType === 'performance' || intent.primaryAction === 'improve') {
            subtasks.push({
                id: `subtask-${Date.now()}-${order++}`,
        ...
        if (intent.requiresDocumentation || intent.requiresNewCode) {
            const implTasks = subtasks.filter(s => s.type === 'implementation');
            const docValidationDeps = implTasks.length > 0 ? implTasks.map(s => s.id) : [];
    ...
    private assignAgentsToSubtasks(subtasks: Subtask[]): AgentAssignment[] {
        ...
        for (const subtask of subtasks) {
            ...
            for (const [agentId, capabilities] of Object.entries(this.agentCapabilities)) {
                let score = 0;
                ...
                if (subtask.type === 'testing' && agentId === 'tester-agent') score += 5;
                if (subtask.type === 'design' && agentId === 'architect-agent') score += 5;
                if (subtask.type === 'implementation' && agentId === 'coder-agent') score += 5;
                if (subtask.type === 'documentation' && agentId === 'documentation-agent') score += 5;
                if (subtask.type === 'security' && agentId === 'security-agent') score += 5;
                if (subtask.type === 'optimization' && agentId === 'optimization-agent') score += 5;
                if (subtask.type === 'validation' && agentId === 'production-readiness') score += 5;
            ...
            if (sortedAgents.length > 0) {
                assignments.push({
                    ...
                    confidence: sortedAgents[0][1] / (subtask.requiredCapabilities.length * 10)
                });
            }
        }
    ...
    private determineToolRequirements(subtasks: Subtask[]): ToolRequirement[] {
        ...
        for (const subtask of subtasks) {
            ...
            if (subtask.requiredCapabilities.includes('best-practices') || 
            ...
            requirements.push({
                ...
                toolSequence: this.determineToolSequence(subtask.type, Array.from(tools))
            });
        }
        ...
        while (processed.size < subtasks.length) {
            ...
            for (const subtask of subtasks) {
                if (!processed.has(subtask.id)) {
                    const depsProcessed = subtask.dependencies.every(dep => processed.has(dep));
                    if (depsProcessed) {
                        const assignment = assignments.find(a => a.subtaskId === subtask.id);
                        const tools = toolRequirements.find(t => t.subtaskId === subtask.id);
    ...
    private extractEntities(taskDescription: string): string[] {
        ...
        const pascalMatches = taskDescription.match(/\b[A-Z][a-zA-Z]+(?:[A-Z][a-zA-Z]+)*\b/g);
        if (pascalMatches) {
    ...
    private determineUrgency(taskDescription: string): 'low' | 'medium' | 'high' | 'critical' {
    ...
    private async detectBuildTool(): Promise<string> {
        if (fs.existsSync(path.join(this.workspaceRoot, 'webpack.config.js'))) return 'webpack';
        if (fs.existsSync(path.join(this.workspaceRoot, 'vite.config.js'))) return 'vite';
        if (fs.existsSync(path.join(this.workspaceRoot, 'rollup.config.js'))) return 'rollup';
        if (fs.existsSync(path.join(this.workspaceRoot, 'esbuild.js'))) return 'esbuild';
        
        return 'npm';
    }
    
    private calculateComplexity(subtasks: Subtask[]): 'low' | 'medium' | 'high' | 'very-high' {
        const score = subtasks.length * 10 + 
    ...
    private calculateConfidence(intent: TaskIntent, similarTasks: SimilarTask[]): number {
    ...
    private adaptSubtasksFromPattern(
        ...
        return patternSubtasks.map((pattern, index) => ({
            ...
            description: pattern.description.replace(/\{target\}/g, intent.targetType),
            type: pattern.type,
    ...
    private findCriticalPath(subtasks: Subtask[]): string[] {
        ...
        const leaves = subtasks.filter(s => 
            !subtasks.some(other => other.dependencies.includes(s.id))
        );
...
interface Subtask {
    id: string;
    description: string;
    type: 'analysis' | 'design' | 'implementation' | 'testing' | 'security' | 'optimization' | 'documentation' | 'validation';
    requiredCapabilities: string[];
    dependencies: string[];
