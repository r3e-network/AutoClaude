...
export class ContextManager {
  ...
  async initialize(): Promise<void> {
    debugLog(`Initializing ContextManager for workspace: ${this.workspacePath}`);
    // Build initial import graph if needed
  ...
  trackFileModification(file: string) {
    this.recentFiles = [
      file,
      ...this.recentFiles.filter((f) => f !== file),
  ...
  private async scoreFiles(
    ...
    scored.slice(0, 10).forEach(({ file, score, reason }) => {
      debugLog(
        `  ${path.relative(this.workspacePath, file)}: ${score} (${reason})`,
      );
    });
