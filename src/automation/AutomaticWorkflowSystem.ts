...
export class AutomaticWorkflowSystem {
  ...
  static getInstance(workspaceRoot: string): AutomaticWorkflowSystem {
    if (!AutomaticWorkflowSystem.instance) {
      AutomaticWorkflowSystem.instance = new AutomaticWorkflowSystem(
        workspaceRoot,
      );
    }
  ...
  private async parseCommand(command: string): Promise<HiveMindTask[]> {
    ...
    if (
      ...
      lowerCommand.includes("documentation")
    ) {
      tasks.push({
        id: taskId(),
    ...
    await this.memorySystem.recordLearning(
      "command-parsing",
      command,
      tasks,
      0.8,
  ...
  private async queueTask(task: HiveMindTask): Promise<void> {
    ...
    const cachedResult = await this.memorySystem.searchCache(
      task.type,
      task.context,
    );
    ...
    while (this.taskQueue.length > 0 && this.isActive) {
      ...
      try {
        ...
        if (result.success) {
          ...
          if (!validationResult.isProductionReady) {
            log.error(
              "Task cannot be completed - code is not production ready",
              {
                taskId: task.id,
        ...
        if (this.currentSession) {
          ...
          await this.memorySystem.updateSession(
            this.currentSession,
            sessionData,
          );
        }
        ...
        log.error("Task processing failed", error as Error, {
          taskId: task.id,
        });
      ...
      // Record metrics
      await this.memorySystem.recordMetric(
        "task_duration",
        task.duration || 0,
        {
          type: task.type,
  ...
  private startAutomaticMonitoring(): void {
    ...
    setInterval(async () => {
      ...
      if (this.taskQueue.length > 50) {
        log.warn("Task queue is getting large", {
  ...
  async stopSession(): Promise<void> {
    ...
    if (this.currentSession) {
      ...
      await this.memorySystem.updateSession(this.currentSession, {
        ended: true,
      });
    }
    ...
    return new Promise((resolve, reject) => {
      const checkInterval = setInterval(() => {
        if (task.status === TaskStatus.COMPLETED) {
          ...
          clearInterval(checkInterval);
          reject(
            new Error(`Task failed: ${task.result?.error || "Unknown error"}`),
          );
        }
      }, 100);
