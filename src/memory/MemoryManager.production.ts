...
export interface ConversionPattern {
    ...
    rust_pattern: string;
    pattern_type: 'type' | 'syntax' | 'idiom' | 'api' | 'file_conversion' | 'validated_conversion';
    confidence: number;
    usage_count: number;
...
class MemoryError extends AutoClaudeError {
    constructor(message: string, code: string, context?: any) {
        super(
            ...
            true,
            message,
            ['Check memory database integrity', 'Restart VS Code', 'Clear memory cache']
        );
    }
}
...
export class MemoryManager {
    ...
    constructor(
        ...
        this.config = {
            dbPath: config?.dbPath || path.join(workspacePath, '.autoclaude', 'memory.db'),
        ...
        try {
            ...
            await new Promise<void>((resolve, reject) => {
                this.db = new sqlite3.Database(
                    this.dbPath,
                    this.readonly ? sqlite3.OPEN_READONLY : sqlite3.OPEN_READWRITE | sqlite3.OPEN_CREATE,
                    (err: Error | null) => {
                        if (err) {
    ...
    private async configureDatabaseSettings(): Promise<void> {
        ...
        await new Promise<void>((resolve, reject) => {
            this.db!.serialize(() => {
                ...
                this.db!.run('PRAGMA cache_size = -64000', (err) => { // 64MB cache
    ...
    private async createTables(): Promise<void> {
        ...
        await this.transaction(async () => {
            ...
            const currentVersion = await this.dbGet('SELECT MAX(version) as version FROM db_version');
            if (!currentVersion || currentVersion.version < MEMORY_DB_VERSION) {
                await this.dbRun('INSERT INTO db_version (version) VALUES (?)', [MEMORY_DB_VERSION]);
            }
        });
    }
    ...
    async transaction<T>(operation: () => Promise<T>): Promise<T> {
        if (this.readonly) {
            throw new MemoryError('Cannot start transaction in readonly mode', 'READONLY_TRANSACTION');
        }
    ...
    async recordPattern(
        ...
        if (csharpPattern.length > this.config.maxPatternLength || 
        ...
        try {
            ...
            throw new MemoryError(
                `Failed to record pattern: ${error.message}`,
    ...
    async findSimilarPatterns(
        ...
        try {
            ...
            return withSimilarity
                ...
                .slice(0, limit)
                .map(({ similarity, ...pattern }: any) => pattern);
                
        } catch (error: any) {
            debugLog(`Failed to find similar patterns: ${error.message}`);
            return [];
        }
    }

    // Project context management with validation
    async getOrCreateProject(projectPath: string, moduleName: string): Promise<ProjectContext> {
        this.ensureInitialized();

        if (!projectPath || !moduleName) {
            throw new MemoryError('Project path and module name are required', 'INVALID_PROJECT_INFO');
        }
        ...
        try {
            ...
            throw new MemoryError(
                `Failed to get/create project: ${error.message}`,
    ...
    async storeAgentMemory(
        ...
        if (!agentId || !key) {
            throw new MemoryError('Agent ID and key are required', 'INVALID_AGENT_MEMORY');
        }
        ...
        try {
            ...
            throw new MemoryError(
                `Failed to store agent memory: ${error.message}`,
     ...
     */
    async getPatterns(category: string, minConfidence: number = 0.7): Promise<Array<{
        id: number;
    ...
    async getPerformanceStats(): Promise<{
        ...
        const stats = {
            ...
            averageQueryTime: this.queryCount > 0 ? this.queryTime / this.queryCount : 0,
            databaseSize: 0,
    ...
    private ensureWritable(): void {
        if (this.readonly) {
            throw new MemoryError('Memory system is in readonly mode', 'READONLY_MODE');
        }
    }
    ...
    async updatePatternSuccess(patternHash: string, success: boolean): Promise<void> {
        ...
        try {
            ...
            throw new MemoryError(
                `Failed to update pattern success: ${error.message}`,
    ...
    async updateProjectProgress(
        ...
        const allowedFields = [
            'conversion_status', 'files_total', 'files_converted',
            'tests_total', 'tests_passing', 'end_time', 'metadata'
        ];
        ...
        try {
            ...
            throw new MemoryError(
                `Failed to update project progress: ${error.message}`,
    ...
    async recallAgentMemory(agentId: string, key: string): Promise<any> {
        ...
        try {
            ...
            throw new MemoryError(
                `Failed to recall agent memory: ${error.message}`,
    ...
    async getAgentMemories(agentId: string, minImportance: number = 0): Promise<AgentMemory[]> {
        this.ensureInitialized();
    ...
    async getTypeMapping(csharpType: string, namespace?: string): Promise<TypeMapping | null> {
        this.ensureInitialized();
        ...
        try {
            ...
            debugLog(`Failed to get type mapping: ${error.message}`);
            return null;
        }
    }
    ...
    async getAllTypeMappings(): Promise<TypeMapping[]> {
        ...
        try {
            ...
            debugLog(`Failed to get all type mappings: ${error.message}`);
            return [];
        }
    }

    // Conversion history
    async recordConversion(history: Omit<ConversionHistory, 'id'>): Promise<void> {
        this.ensureInitialized();
        this.ensureWritable();

        if (!history.project_id || !history.source_file || !history.target_file) {
            throw new MemoryError('Project ID, source file, and target file are required', 'INVALID_CONVERSION_HISTORY');
        }
        ...
        try {
            ...
            throw new MemoryError(
                `Failed to record conversion: ${error.message}`,
    ...
    async getConversionHistory(projectId: number): Promise<ConversionHistory[]> {
        ...
        try {
            ...
            debugLog(`Failed to get conversion history: ${error.message}`);
            return [];
        }
    }
    ...
    async getConversionStats(): Promise<any> {
        ...
        try {
            ...
            debugLog(`Failed to get conversion stats: ${error.message}`);
            return {
                overall: {},
                patterns: {},
                topPatterns: []
    ...
    async pruneOldMemory(daysToKeep: number = 30): Promise<void> {
        ...
        try {
            ...
            throw new MemoryError(
                `Failed to prune old memory: ${error.message}`,
                'MEMORY_PRUNE_FAILED',
                { daysToKeep, error }
            );
        }
    ...
    async exportMemory(outputPath: string): Promise<void> {
        ...
        try {
            const data = {
                ...
                patterns: await this.dbAll('SELECT * FROM conversion_patterns ORDER BY confidence DESC'),
                typeMappings: await this.dbAll('SELECT * FROM type_mappings ORDER BY conversion_complexity'),
                projects: await this.dbAll('SELECT * FROM project_context ORDER BY created_at DESC'),
            ...
            throw new MemoryError(
                `Failed to export memory: ${error.message}`,
    ...
    private async loadBuiltinMappings(): Promise<void> {
        const builtinMappings: Partial<TypeMapping>[] = [
            // Basic types
            { csharp_type: 'int', rust_type: 'i32', conversion_complexity: 1, confidence: 1.0 },
            { csharp_type: 'uint', rust_type: 'u32', conversion_complexity: 1, confidence: 1.0 },
            { csharp_type: 'long', rust_type: 'i64', conversion_complexity: 1, confidence: 1.0 },
            { csharp_type: 'ulong', rust_type: 'u64', conversion_complexity: 1, confidence: 1.0 },
            { csharp_type: 'short', rust_type: 'i16', conversion_complexity: 1, confidence: 1.0 },
            { csharp_type: 'ushort', rust_type: 'u16', conversion_complexity: 1, confidence: 1.0 },
            { csharp_type: 'byte', rust_type: 'u8', conversion_complexity: 1, confidence: 1.0 },
            { csharp_type: 'sbyte', rust_type: 'i8', conversion_complexity: 1, confidence: 1.0 },
            { csharp_type: 'bool', rust_type: 'bool', conversion_complexity: 1, confidence: 1.0 },
            { csharp_type: 'char', rust_type: 'char', conversion_complexity: 1, confidence: 1.0 },
            { csharp_type: 'float', rust_type: 'f32', conversion_complexity: 1, confidence: 1.0 },
            { csharp_type: 'double', rust_type: 'f64', conversion_complexity: 1, confidence: 1.0 },
            { csharp_type: 'decimal', rust_type: 'rust_decimal::Decimal', conversion_complexity: 2, requires_import: 'rust_decimal', confidence: 0.9 },
            { csharp_type: 'string', rust_type: 'String', conversion_complexity: 2, confidence: 0.95 },
            
            // Collections
            { csharp_type: 'List<T>', rust_type: 'Vec<T>', is_generic: true, conversion_complexity: 2, confidence: 0.95 },
            { csharp_type: 'Dictionary<K,V>', rust_type: 'HashMap<K,V>', is_generic: true, conversion_complexity: 3, requires_import: 'std::collections::HashMap', confidence: 0.9 },
            { csharp_type: 'HashSet<T>', rust_type: 'HashSet<T>', is_generic: true, conversion_complexity: 2, requires_import: 'std::collections::HashSet', confidence: 0.9 },
            { csharp_type: 'Queue<T>', rust_type: 'VecDeque<T>', is_generic: true, conversion_complexity: 2, requires_import: 'std::collections::VecDeque', confidence: 0.85 },
            { csharp_type: 'Stack<T>', rust_type: 'Vec<T>', is_generic: true, conversion_complexity: 2, notes: 'Use Vec with push/pop', confidence: 0.8 },
            { csharp_type: 'IEnumerable<T>', rust_type: 'impl Iterator<Item=T>', is_generic: true, conversion_complexity: 3, confidence: 0.7 },
            { csharp_type: 'IList<T>', rust_type: 'Vec<T>', is_generic: true, conversion_complexity: 2, confidence: 0.85 },
            
            // Neo-specific types
            { csharp_type: 'BigInteger', rust_type: 'num_bigint::BigInt', namespace: 'System.Numerics', conversion_complexity: 2, requires_import: 'num_bigint', confidence: 0.9 },
            { csharp_type: 'UInt160', rust_type: 'U160', namespace: 'Neo', conversion_complexity: 2, confidence: 0.95 },
            { csharp_type: 'UInt256', rust_type: 'U256', namespace: 'Neo', conversion_complexity: 2, confidence: 0.95 },
            { csharp_type: 'ECPoint', rust_type: 'PublicKey', namespace: 'Neo.Cryptography', conversion_complexity: 3, confidence: 0.85 },
            { csharp_type: 'StackItem', rust_type: 'StackItem', namespace: 'Neo.VM', conversion_complexity: 3, confidence: 0.9 },
            
            // Common patterns
            { csharp_type: 'Task<T>', rust_type: 'Pin<Box<dyn Future<Output = T>>>', is_generic: true, conversion_complexity: 4, requires_import: 'std::pin::Pin, std::future::Future', confidence: 0.7 },
            { csharp_type: 'Task', rust_type: 'Pin<Box<dyn Future<Output = ()>>>', conversion_complexity: 4, requires_import: 'std::pin::Pin, std::future::Future', confidence: 0.7 },
            { csharp_type: 'Result<T>', rust_type: 'Result<T, Box<dyn std::error::Error>>', is_generic: true, conversion_complexity: 2, confidence: 0.9 }
        ];
...
export function getMemoryManager(workspacePath: string, config?: MemoryConfig): MemoryManager {
...
export function closeAllMemoryManagers(): Promise<void[]> {
    const promises = Array.from(memoryManagers.values()).map(manager => manager.close());
