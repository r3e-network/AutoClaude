...
export class SQLiteMemorySystem {
    ...
    async recordPattern(data: {
        ...
        if (existing) {
            ...
            const newSuccessRate = ((existing.success_rate * existing.frequency) + (data.success ? 1 : 0)) / newFrequency;
            const newAvgDuration = ((existing.average_duration * existing.frequency) + data.duration) / newFrequency;
            ...
            await this.db.run(
                ...
                now,
                now,
                JSON.stringify({ ...JSON.parse(existing.metadata || '{}'), lastContext: data.context }),
                existing.id
            );
        } else {
            // Create new pattern
    ...
    async searchCache(type: string, context: any): Promise<any | null> {
    ...
    async cacheResult(type: string, context: any, result: any, ttl?: number): Promise<void> {
        if (!this.db) return;
    ...
    async recordTask(task: any): Promise<void> {
        if (!this.db) return;
    ...
    async recordLearning(category: string, input: any, output: any, confidence: number): Promise<void> {
        if (!this.db) return;
    ...
    async recordMetric(type: string, value: number, tags?: Record<string, any>): Promise<void> {
        if (!this.db) return;
