...
export class ProductionReadinessValidator {
  ...
  private readonly FORBIDDEN_PATTERNS = [
    ...
    {
      pattern: /\/\/\s*OPTIMIZE\s*:/gi,
      message: "OPTIMIZE comment found",
      severity: "warning",
    },
    {
      pattern: /\/\/\s*REFACTOR\s*:/gi,
      message: "REFACTOR comment found",
      severity: "warning",
    },
    ...
    {
      pattern: /mock(?!ing|ito)/gi,
      message: "Mock implementation found",
      severity: "error",
    },
    {
      pattern: /stub(?!born)/gi,
      message: "Stub implementation found",
      severity: "error",
    },
    {
      pattern: /dummy/gi,
      message: "Dummy implementation found",
      severity: "error",
    },
    {
      pattern: /fake(?!r)/gi,
      message: "Fake implementation found",
      severity: "error",
    },
    {
      pattern: /temp(?!late|erature)/gi,
      message: "Temporary implementation found",
      severity: "warning",
    },
    {
      pattern: /test(?!ing|s|ed|able)\s+data/gi,
    ...
    {
      pattern: /simplified/gi,
      message: "Simplified implementation found",
      severity: "error",
    },
    {
      pattern: /simplistic/gi,
      message: "Simplistic implementation found",
      severity: "error",
    },
    {
      pattern: /basic\s+implementation/gi,
      message: "Basic implementation found",
      severity: "warning",
    },
    {
      pattern: /minimal\s+implementation/gi,
      message: "Minimal implementation found",
      severity: "warning",
    },
    ...
    // Not implemented patterns
    {
      ...
      message: "Not implemented found",
      severity: "error",
    },
    {
      pattern: /throw\s+new\s+Error\s*\(\s*['"`]Not\s+implemented/gi,
      message: "Not implemented error found",
      severity: "error",
    },
    {
      pattern: /return\s+null\s*;\s*\/\/\s*placeholder/gi,
      message: "Placeholder return found",
      severity: "error",
    },
    {
      pattern: /return\s+undefined\s*;\s*\/\/\s*placeholder/gi,
      message: "Placeholder return found",
      severity: "error",
    },
    {
      pattern: /\/\/\s*implement\s+later/gi,
      message: "Implement later comment found",
      severity: "error",
    },
    {
      pattern: /\/\/\s*finish\s+this/gi,
    ...
    {
      pattern: /catch\s*\(\s*\w*\s*\)\s*{\s*\/\/\s*ignore/gi,
      message: "Ignored error found",
      severity: "error",
    },
    {
      pattern: /\.catch\s*\(\s*\(\s*\)\s*=>\s*{\s*}\s*\)/g,
      message: "Empty promise catch found",
      severity: "error",
    },
    ...
    },
    {
      ...
      message: "TypeScript any cast found",
      severity: "warning",
    },
    ...
    // Incomplete implementations
    {
      ...
      message: "Coming soon found",
      severity: "error",
    },
    {
      pattern: /work\s+in\s+progress/gi,
      message: "Work in progress found",
      severity: "error",
    },
    {
      pattern: /under\s+construction/gi,
      message: "Under construction found",
      severity: "error",
    },
    {
      pattern: /to\s+be\s+implemented/gi,
      message: "To be implemented found",
      severity: "error",
    },
    ...
    {
      pattern: /confirm\s*\(/g,
      message: "Confirm statement found",
      severity: "warning",
    },

    // Comments indicating problems
    {
      pattern: /\/\/\s*this\s+is\s+bad/gi,
      message: "Bad code comment found",
      severity: "error",
    },
    {
      pattern: /\/\/\s*quick\s+and\s+dirty/gi,
      message: "Quick and dirty comment found",
      severity: "error",
    },
    {
      pattern: /\/\/\s*temporary\s+fix/gi,
      message: "Temporary fix comment found",
      severity: "error",
    },
    {
      pattern: /\/\/\s*workaround/gi,
      message: "Workaround comment found",
      severity: "warning",
    },

    // Incomplete documentation
    {
      pattern: /@param\s+\{[^}]*\}\s*$/gm,
      message: "Incomplete param documentation",
      severity: "warning",
    },
    {
      pattern: /@returns?\s*$/gm,
      message: "Incomplete return documentation",
      severity: "warning",
    },

    // AI/Assistant references (should not be in code)
    {
      pattern: /claude|ai\s+assistant|generated\s+by\s+ai/gi,
      message: "AI reference found in code",
      severity: "error",
    },
  ];
  ...
  async validateProductionReadiness(
    ...
    try {
      ...
      if (
        patternResult.errors.length > 0 ||
        patternResult.criticalIssues.length > 0
      ) {
        result.isProductionReady = false;
      }
      ...
      if (detectedLanguages.length > 0) {
        log.info("Detected project languages", {
          languages: detectedLanguages,
        });
        const languageResult =
          await this.runLanguageSpecificChecks(detectedLanguages);
   ...
   */
  private async checkFileForPatterns(
    filePath: string,
      ...
      for (const forbiddenPattern of this.FORBIDDEN_PATTERNS) {
        ...
        for (const match of matches) {
          ...
          issues.push({
            ...
            severity: forbiddenPattern.severity as
              | "error"
              | "warning"
              | "critical",
          });
  ...
  private async runSubAgentCheck(): Promise<CheckResult> {
    try {
      ...
      const result = await this.subAgentRunner.runSingleAgent(
  ...
  formatValidationResult(result: ValidationResult): string {
    ...
    if (!result.isProductionReady) {
      ...
      lines.push(
        "  2. Remove all TODOs, placeholders, and incomplete implementations",
      );
   ...
   */
  private async runLanguageSpecificChecks(
    languages: string[],
  ): Promise<CheckResult> {
    ...
    for (const language of languages) {
      ...
      if (agentId) {
        ...
        try {
          ...
          log.warn(
            `${language} quality check not available or failed`,
            error as Error,
          );
