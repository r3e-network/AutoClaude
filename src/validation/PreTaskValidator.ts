...
export class PreTaskValidator {
     ...
     */
    private async validateTDDRequirements(task: HiveMindTask): Promise<ValidationResult> {
        const result: ValidationResult = {
            passed: true,
            errors: [],
            warnings: [],
        ...
        try {
            ...
            const tddCheck = await this.subAgentRunner.runSingleAgent('tdd-enforcement');
     ...
     */
    private async validateDDDRequirements(task: HiveMindTask): Promise<ValidationResult> {
        const result: ValidationResult = {
            passed: true,
            errors: [],
            warnings: [],
        ...
        try {
            ...
            const targetEntity = task.context?.entity || task.context?.targetType;
            if (targetEntity) {
                const hasDocumentation = await this.checkForCorrespondingDocumentation(targetEntity);
                if (!hasDocumentation) {
                    result.passed = false;
                    result.blockers.push({
                        type: 'ddd-violation',
            ...
            const dddCheck = await this.subAgentRunner.runSingleAgent('ddd-enforcement');
     ...
     */
    private async validateTestsWillFail(task: HiveMindTask): Promise<ValidationResult> {
        const result: ValidationResult = {
            passed: true,
            errors: [],
            warnings: [],
        ...
        try {
            ...
            const targetEntity = task.context?.entity || task.context?.targetType;
            if (targetEntity) {
                const implementationExists = await this.checkForExistingImplementation(targetEntity);
     ...
     */
    private async validateDocumentationComprehensive(task: HiveMindTask): Promise<ValidationResult> {
        const result: ValidationResult = {
            passed: true,
            errors: [],
            warnings: [],
        ...
        try {
            ...
            if (context?.includeErrorHandling) requiredElements.push('error handling');
     ...
     */
    private async checkForCorrespondingDocumentation(entity: string): Promise<boolean> {
        try {
            ...
            for (const pattern of sourcePatterns) {
                const files = await vscode.workspace.findFiles(pattern, '**/node_modules/**');
                for (const file of files) {
                    if (file.fsPath.includes('.test.') || file.fsPath.includes('.spec.')) continue;
            ...
            for (const pattern of docPatterns) {
                ...
                if (files.length > 0) {
                    ...
                    for (const file of files) {
                        const content = await fs.promises.readFile(file.fsPath, 'utf8');
                        if (content.trim().length > 100) { // Minimum documentation length
                            return true;
                        }
                    }
                }
     ...
     */
    private async checkForExistingImplementation(entity: string): Promise<boolean> {
        try {
            const sourcePatterns = [
                `**/*${entity}*.*`,
                `**/${entity}.*`
            ...
            for (const pattern of sourcePatterns) {
                ...
                for (const file of files) {
                    ...
                    if (/(?:function|class|const.*=>)/.test(content) && content.trim().length > 200) {
                        return true;
                    }
                }
            }
