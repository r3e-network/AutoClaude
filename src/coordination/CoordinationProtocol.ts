...
export class CoordinationProtocol {
  ...
  constructor(workspacePath: string) {
    ...
    this.activeWorkFile = path.join(
      this.coordinationDir,
      "active_work_registry.json",
    );
    this.completedWorkFile = path.join(
      this.coordinationDir,
      "completed_work_log.json",
    );
    this.plannedWorkFile = path.join(
      this.coordinationDir,
  ...
  async requestWork(
    ...
    debugLog(
      `Agent ${agentId} requesting work with capabilities: ${capabilities.join(", ")}`,
    );
  ...
  async planWork(
    agentId: string,
    plan: Omit<WorkPlan, "id" | "status" | "createdAt">,
  ): Promise<WorkPlan> {
  ...
  async completeWork(
    ...
    if (!work || work.agentId !== agentId) {
      throw new Error(
        `Work ${workId} not found or not assigned to agent ${agentId}`,
      );
    }
  ...
  private async canAgentTakeWork(
    agentId: string,
    plan: WorkPlan,
  ): Promise<boolean> {
  ...
  private async loadState(): Promise<void> {
    ...
    try {
      const completedData = await fsAsync.readFile(
        this.completedWorkFile,
        "utf-8",
      );
  ...
  private async saveState(): Promise<void> {
    ...
    await fsAsync.writeFile(
      this.completedWorkFile,
      JSON.stringify(this.state.completedWorkLog, null, 2),
    );

    // Save planned work queue
    await fsAsync.writeFile(
      this.plannedWorkFile,
      JSON.stringify(this.state.plannedWorkQueue, null, 2),
    );
  }
