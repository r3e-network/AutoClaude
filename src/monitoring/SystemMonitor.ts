...
export class SystemMonitor {
    ...
    private async collectDiskMetrics(): Promise<DiskMetrics> {
        try {
            ...
            const usagePercentage = ((totalSpace - availableSpace) / totalSpace) * 100;
    ...
    private async collectAutoClaudeMetrics(): Promise<AutoClaudeMetrics> {
        try {
            ...
            if (agentCoordinator) {
                ...
                activeAgents = agentStatus.filter((a: any) => a.status === 'busy').length;
    ...
    private getRecentErrorCount(): number {
        ...
        return this.alerts.filter(alert => 
            alert.type === 'error' && alert.timestamp > oneMinuteAgo
        ).length;
    }
    ...
    private async checkAlerts(metrics: SystemMetrics): Promise<void> {
        ...
        // Error rate alert
        if (metrics.autoClaudeMetrics.errorCount > this.config.alerts.errorRateThreshold) {
            alerts.push({
                id: `errors-${Date.now()}`,
                type: 'error',
    ...
    async showMonitoringDashboard(): Promise<void> {
        ...
        if (activeAlerts.length > 0) {
            ...
            activeAlerts.forEach(alert => {
                ...
                dashboardContent.push(`- ${icon} **${alert.title}**: ${alert.message} (${alert.timestamp.toLocaleString()})`);
            });
        }
    ...
    private performCleanup(): void {
        ...
        debugLog(`Cleanup completed: ${this.metrics.length} metrics, ${this.alerts.length} alerts retained`);
    }
