...
export class WorkDistributor {
  ...
  async loadWorkFromFile(filePath: string): Promise<void> {
    ...
    try {
      ...
      if (lines.length > maxLines) {
        debugLog(
          `Warning: File has ${lines.length} lines, processing only first ${maxLines}`,
        );
      }
  ...
  async getWorkChunk(
    ...
    debugLog(
      `Assigned chunk ${chunk.id} with ${chunkItems.length} items to agent ${agentId}`,
    );
    return chunk;
  }
  ...
  async markChunkCompleted(
    ...
    await this.saveState();
    debugLog(
      `Marked chunk ${chunkId} as completed with ${completedItems.length} items`,
    );
  }
  ...
  private calculateDynamicChunkSize(): number {
    const totalPending = Array.from(this.workItems.values()).filter(
  ...
  async appendCompletedMarker(
    ...
    try {
      ...
      debugLog(
        `Updated work file with ${completedIds.length} completed markers`,
      );
    } catch (error) {
  ...
  getStatistics(): {
    ...
    const activeChunks = Array.from(this.chunks.values()).filter(
      (chunk) => !chunk.completedAt,
    ).length;
