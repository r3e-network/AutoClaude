...
export class AutoWorkDetector {
  ...
  async detectWork(): Promise<WorkDetectionResult> {
    ...
    if (problems.length > 0) {
      const problemsFile = path.join(
        this.workspacePath,
        ".autoclaude",
        "detected_problems.txt",
      );
      ...
      if (detection.hasWork) {
        ...
        if (action === "Yes") {
          ...
          if (autoScale && currentAgents < detection.suggestedAgents) {
            debugLog(
              `Auto-scaling from ${currentAgents} to ${detection.suggestedAgents} agents`,
            );
            await this.orchestrator.startAgents(
              detection.suggestedAgents - currentAgents,
            );
          }
          ...
          for (const agent of agents) {
            if (agent.status === "idle" || agent.status === "ready") {
              ...
              if (chunk) {
                ...
                debugLog(
                  `Distributing ${chunk.items.length} items to ${agent.name}`,
                );
              }
            }
          }
  ...
  private generateRecommendations(
    ...
    if (detection.workCount > 100) {
      recommendations.push(
        "- Consider running agents overnight for large workload",
      );
    }

    if (detection.workTypes.includes("test-failures")) {
      recommendations.push(
        "- Fix test failures first to ensure code stability",
      );
    }
    ...
    if (!detection.techStack) {
      recommendations.push(
        "- Configure tech stack for better problem detection",
      );
    }
