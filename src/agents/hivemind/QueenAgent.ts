...
import { HiveMindAgent, AgentRole, TaskPriority, HiveMindTask, HiveMindResult } from './types';
...
export class QueenAgent implements HiveMindAgent {
    ...
    capabilities = ['orchestration', 'task-decomposition', 'agent-assignment', 'priority-management'];
    ...
    async initialize(): Promise<void> {
        log.info('Queen Agent initializing...', { workspaceRoot: this.workspaceRoot });
    ...
    async execute(task: any): Promise<any> {
        log.info('Queen Agent executing task', { taskId: task.id, type: task.type });
        ...
        try {
            ...
            log.error('Queen Agent execution failed', error as Error, { taskId: task.id });
            throw error;
        }
    }
    ...
    private async initializeSwarm(): Promise<void> {
        ...
        for (const agent of agents) {
            try {
                ...
                log.error('Failed to initialize agent', error as Error, { agentId: agent.id });
            }
        }
    ...
    private async analyzeTaskComplexity(task: any): Promise<{ isComplex: boolean; score: number }> {
    ...
    private async decomposeTask(task: any): Promise<any[]> {
        ...
        if (task.type === 'feature-implementation') {
            subtasks.push(
                { ...task, type: 'architecture-design', priority: TaskPriority.HIGH },
                { ...task, type: 'code-implementation', priority: TaskPriority.MEDIUM },
                { ...task, type: 'test-creation', priority: TaskPriority.MEDIUM },
                { ...task, type: 'documentation', priority: TaskPriority.LOW }
            );
        }
    ...
    private async orchestrateSubtasks(subtasks: any[]): Promise<any> {
    ...
    private async selectBestAgent(task: any): Promise<HiveMindAgent> {
    ...
    private calculateAgentScore(agent: HiveMindAgent, task: any): number {
    ...
    private async delegateToAgent(agent: HiveMindAgent, task: any): Promise<any> {
        log.info('Delegating task to agent', { agentId: agent.id, taskId: task.id });
        
        // Track delegation
        this.agentCoordinator.assignTask(agent.id, task);
    ...
    private groupByDependencies(tasks: any[]): any[][] {
    ...
    private hasDependency(task1: any, task2: any): boolean {
    ...
    private async restartAgent(agent: HiveMindAgent): Promise<void> {
        try {
            ...
            log.error('Failed to restart agent', error as Error, { agentId: agent.id });
    ...
    private async redistributeTasks(overloadedAgentId: string): Promise<void> {
        ...
        for (const task of redistributable.slice(0, 2)) { // Move at most 2 tasks
            const newAgent = await this.selectBestAgent(task);
            if (newAgent.id !== overloadedAgentId) {
                await this.agentCoordinator.reassignTask(task.id, overloadedAgentId, newAgent.id);
...
class TaskQueue {
    ...
    getRecentCompleted(limit = 10): any[] {
