...
export default class DocumentationAgent implements HiveMindAgent {
    ...
    async execute(task: HiveMindTask): Promise<HiveMindResult> {
        log.info('Documentation Agent executing task', { taskId: task.id, type: task.type });
        ...
        try {
            ...
            log.error('Documentation Agent execution failed', error as Error);
            return {
                success: false,
    ...
    private async generateApiDocumentation(task: HiveMindTask): Promise<HiveMindResult> {
        ...
        const artifacts = [
            ...
            },
            {
                ...
                path: path.join(this.workspaceRoot, 'docs', 'api', 'postman-collection.json')
            }
        ];
        ...
        return {
            ...
            data: {
                ...
                examplesGenerated: documentation.examples.length
            },
            artifacts,
            metrics: {
    ...
    private async generateCodeDocumentation(task: HiveMindTask): Promise<HiveMindResult> {
        ...
        for (const module of documentation.modules) {
            artifacts.push({
                ...
                path: path.join(this.workspaceRoot, 'docs', 'code', `${module.name}.md`)
            });
        }
    ...
    private async createUserGuides(task: HiveMindTask): Promise<HiveMindResult> {
        ...
        artifacts.push({
            ...
            path: path.join(this.workspaceRoot, 'docs', 'user-guide', 'quick-start.md')
        });
        ...
        for (const guide of guides.featureGuides) {
            artifacts.push({
                ...
                path: path.join(this.workspaceRoot, 'docs', 'user-guide', `${guide.feature}.md`)
            });
        }
    ...
    private async generateReadme(task: HiveMindTask): Promise<HiveMindResult> {
        ...
        return {
            success: true,
            data: {
                sectionsGenerated: readme.sections.length,
                badgesIncluded: readme.badges.length,
                installationSteps: readme.installationSteps.length
            },
            artifacts: [{
                type: 'documentation' as const,
    ...
    private async maintainChangelog(task: HiveMindTask): Promise<HiveMindResult> {
        ...
        const updatedChangelog = await this.updateChangelog(currentChangelog, gitHistory, task.context);
    ...
    private async createTechnicalSpecs(task: HiveMindTask): Promise<HiveMindResult> {
    ...
    private async createTutorials(task: HiveMindTask): Promise<HiveMindResult> {
        ...
        for (const tutorial of tutorials.tutorials) {
            artifacts.push({
                ...
                path: path.join(this.workspaceRoot, 'docs', 'tutorials', `${tutorial.slug}.md`)
            });
        }
    ...
    private async addComprehensiveDocumentation(task: HiveMindTask): Promise<HiveMindResult> {
    ...
    private async genericDocumentationTask(task: HiveMindTask): Promise<HiveMindResult> {
        const documentation = await this.generateGenericDocumentation(task.description);
        ...
        return {
            ...
            artifacts: [{
                ...
                path: path.join(this.workspaceRoot, 'docs', `${task.description.toLowerCase().replace(/\s+/g, '-')}.md`)
            }],
            metrics: {
                duration: Date.now() - (task.startedAt || Date.now())
            }
    ...
    private extractInterfaceProperties(content: string, interfaceName: string): string[] {
    ...
    private async generateApiDocs(apiSpec: any): Promise<any> {
    ...
    private generateOpenApiSpec(apiSpec: any): string {
        const spec = {
            openapi: '3.0.0',
            info: {
    ...
    private generateApiReadme(apiSpec: any): string {
...
${apiSpec.endpoints.map((endpoint: any) => `
...
`;
    ...
    private generatePostmanCollection(apiSpec: any): any {
        return {
            info: {
                name: 'API Collection',
                description: 'Auto-generated Postman collection'
            },
            item: apiSpec.endpoints.map((endpoint: any) => ({
    ...
    private generateApiExamples(apiSpec: any): any[] {
    ...
    private async generateCodeDocs(codeAnalysis: any): Promise<any> {
    ...
    private generateCodeOverview(analysis: any): string {
...
### Modules Overview
${analysis.modules.map((module: any) => `
#### ${module.name}
- **Path**: ${module.path}
...
### Main Functions
${analysis.functions.slice(0, 20).map((func: any) => `
#### ${func.name}
- **File**: ${func.file}
...
### Main Classes
${analysis.classes.slice(0, 10).map((cls: any) => `
#### ${cls.name}
- **File**: ${cls.file}
...
`;
    ...
    private generateModuleDocs(modules: any[]): any[] {
...
`
    ...
    private generateInlineDocUpdates(analysis: any): any[] {
    ...
    private async identifyUserPersonas(): Promise<any[]> {
        return [
            ...
            },
            {
                ...
                needs: ['User interface guides', 'Feature tutorials', 'Troubleshooting']
            },
            {
                name: 'Administrator',
    ...
    private async generateUserGuides(personas: any[], features: any[]): Promise<any> {
    ...
    private generateMainUserGuide(features: any[]): string {
        return `# User Guide
...
`;
    ...
    private generateFeatureGuide(feature: any): string {
...
`;
    ...
    private async analyzeProject(): Promise<any> {
        ...
        try {
            ...
            if (packageFiles.length > 0) {
                const document = await vscode.workspace.openTextDocument(packageFiles[0]);
                packageJson = JSON.parse(document.getText());
            }
        } catch (error) {
            log.warn('Failed to read package.json', error as Error);
        }
    ...
    private async createReadme(projectInfo: any): Promise<any> {
...
## Scripts
${Object.entries(projectInfo.scripts).map(([name, script]) => `- \`npm run ${name}\`: ${script}`).join('\n')}
...
`;
    ...
    private generateBadges(projectInfo: any): any[] {
        const badges = [];
        ...
        badges.push({
            name: 'build',
            markdown: '![Build Status](https://img.shields.io/badge/build-passing-brightgreen.svg)'
        });
        
        return badges;
    }
    
    private generateInstallationSteps(projectInfo: any): string[] {
        const steps = [
    ...
    private async readCurrentChangelog(): Promise<string> {
        try {
            const changelogFiles = await vscode.workspace.findFiles('**/CHANGELOG.md', null, 1);
            if (changelogFiles.length > 0) {
                const document = await vscode.workspace.openTextDocument(changelogFiles[0]);
    ...
    private async analyzeGitHistory(): Promise<any[]> {
        ...
        return [
            ...
            { hash: 'ghi789', message: 'docs: update documentation', type: 'documentation' }
        ];
    }
    
    private async updateChangelog(currentChangelog: string, gitHistory: any[], context: any): Promise<any> {
...
### Added
${newEntries.filter(e => e.type === 'feature').map(e => `- ${e.message}`).join('\n')}

### Fixed
${newEntries.filter(e => e.type === 'bugfix').map(e => `- ${e.message}`).join('\n')}

### Changed
${newEntries.filter(e => e.type === 'change').map(e => `- ${e.message}`).join('\n')}
...
`;
    ...
    private async generateTechnicalSpecs(architecture: any, requirements: any[]): Promise<any> {
    ...
    private generateArchitectureSpec(architecture: any): string {
...
`;
    ...
    private generateRequirementsSpec(requirements: any[]): string {
...
`;
    ...
    private generateDesignSpec(architecture: any, requirements: any[]): string {
...
`;
    ...
    private async generateTutorials(topics: any[]): Promise<any> {
        ...
        const totalEstimatedTime = topics.reduce((acc, t) => acc + parseInt(t.estimatedTime), 0);
    ...
    private generateTutorialContent(topic: any): string {
        return `# ${topic.title}
...
`;
    ...
    private generateTutorialIndex(tutorials: any[]): string {
        return `# Tutorials
...
`;
    ...
    private async generateGenericDocumentation(description: string): Promise<string> {
        return `# ${description}
