...
export class ParallelAgentOrchestrator {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      throw new Error(
        "tmux is not installed. Please install tmux to use parallel agents.",
      );
    }
  ...
  async startAgents(count: number = this.config.defaultAgents): Promise<void> {
    ...
    for (let i = 0; i < count; i++) {
      ...
      try {
        ...
        if (launchSuccess) {
          ...
          this.adaptiveStagger = Math.max(
            this.config.staggerDelay,
            this.adaptiveStagger / 2,
          );
        } else {
    ...
    vscode.window.showInformationMessage(
      `Started ${count} parallel Claude agents`,
    );
  }
  ...
  private async createTmuxSession(): Promise<void> {
    ...
    await execAsync(
      `tmux send-keys -t ${this.tmuxSession}:controller 'echo "Claude Agent Farm Controller"' C-m`,
    );
  ...
  private async launchAgent(agentId: number): Promise<boolean> {
    ...
    try {
      ...
      await execAsync(
        `tmux new-window -t ${this.tmuxSession} -n ${windowName}`,
      );
      ...
      const heartbeatFile = path.join(
        this.heartbeatDir,
        `agent${agentId.toString().padStart(2, "0")}.heartbeat`,
      );
  ...
  private buildClaudeCommand(agentId: number): string {
    ...
    const skipPermissions = config.session.skipPermissions
  ...
  private async updatePaneTitle(agent: AgentConfig): Promise<void> {
    const contextWarning =
      agent.contextUsage <= this.config.contextThreshold ? "⚠️" : "";
  ...
  private async checkAgentHealth(): Promise<void> {
    for (const [id, agent] of this.agents) {
      try {
        ...
        const heartbeatFile = path.join(
          this.heartbeatDir,
          `agent${agent.id.padStart(2, "0")}.heartbeat`,
        );
        ...
        if (heartbeatAge > 120000 && agent.status !== "disabled") {
          debugLog(
            `${agent.name} appears stuck (no heartbeat for ${Math.floor(heartbeatAge / 1000)}s)`,
          );
          agent.status = "error";
          agent.errors++;
        ...
        if (contextMatch) {
          ...
          if (
            agent.contextUsage <= this.config.contextThreshold &&
            agent.status === "working"
          ) {
            debugLog(
              `${agent.name} context low (${agent.contextUsage}%), clearing...`,
            );
  ...
  private async restartAgent(agent: AgentConfig): Promise<void> {
    ...
    const backoffDelay =
      Math.min(10 * Math.pow(2, agent.restartCount - 1), 300) * 1000;
  ...
  async distributeTask(task: any): Promise<string[]> {
    ...
    if (idleAgents.length === 0) {
      debugLog("No idle agents available for task distribution");
      return assignedAgents;
    }

    // Distribute task to the first available idle agent
    const agent = idleAgents[0];
    await this.sendWorkToAgent(agent.id, task.description || "Task assigned");
  ...
  /**
   * Wait for task completion on a specific agent
   */
  async waitForTaskCompletion(agentId: string, taskId: string): Promise<{ success: boolean }> {
    const agent = this.agents.get(agentId);
    if (!agent) {
      return { success: false };
    }

    // Wait for agent to become idle (indicating task completion)
  ...
  private async stopAgent(agentId: string): Promise<void> {
    ...
    const heartbeatFile = path.join(
      this.heartbeatDir,
      `agent${agentId.padStart(2, "0")}.heartbeat`,
    );
    try {
