...
export class ValidatorAgent implements Agent {
    ...
    private validateCSharpSyntax(
        ...
        lines.forEach((line, index) => {
            ...
            if (line.includes('public class') && !line.includes('{') && !lines[index + 1]?.trim().startsWith('{')) {
                errors.push({
                    line: lineNum,
            ...
            if (line.includes('UInt160') || line.includes('UInt256')) {
                suggestions.push(`Line ${lineNum}: Consider using appropriate Rust equivalent for Neo types`);
            }
        });
    ...
    private validateRustSyntax(
        ...
        lines.forEach((line, index) => {
            ...
            if (line.includes('struct') && line.includes('pub') && !line.includes('{') && !lines[index + 1]?.trim().startsWith('{')) {
                errors.push({
                    line: lineNum,
                    column: line.indexOf('struct') + 1,
                    message: 'Struct declaration should be followed by opening brace or semicolon',
                    severity: 'error'
                });
            }

            // Check for missing 'pub' keyword where needed
            if (line.includes('fn ') && !line.includes('pub fn') && !line.trim().startsWith('//')) {
                errors.push({
                    line: lineNum,
    ...
    private async validateNeoCompatibility(input: { 
        ...
        const neoTypes = [
            ...
            'InteropService', 'Script', 'Witness', 'Transaction', 'Block'
        ];
        ...
        for (const neoType of neoTypes) {
            if (originalCSharp.includes(neoType)) {
                ...
                if (!convertedRust.includes(rustEquivalent)) {
                    ...
                    recommendations.push(`Convert ${neoType} to ${rustEquivalent} for Neo compatibility`);
                }
            }
        }
        ...
        // Check for Neo-specific patterns
        if (originalCSharp.includes('ApplicationEngine') && !convertedRust.includes('ApplicationEngine')) {
            issues.push({
                type: 'missing_engine_integration',
                description: 'ApplicationEngine integration not found in Rust conversion',
                severity: 'critical'
            });
    ...
    private async analyzeCode(input: { 
        code: string; 
        language: 'csharp' | 'rust';
        analysisType?: 'complexity' | 'performance' | 'security' | 'maintainability';
    }): Promise<{
        complexity: number;
        ...
        // Analyze complexity
        lines.forEach(line => {
            if (line.includes('if') || line.includes('else') || line.includes('while') || line.includes('for')) {
                complexity++;
            }
        ...
        if (longMethods > 0) {
            ...
            recommendations.push('Consider breaking down long lines for better readability');
        }

        // Analyze performance
        if (code.includes('String.') && language === 'csharp') {
            performance -= 10;
            recommendations.push('Consider using StringBuilder for multiple string operations');
        }

        // Analyze security
        if (code.includes('unsafe') && language === 'rust') {
            security -= 20;
            recommendations.push('Review unsafe code blocks for potential security issues');
        }
    ...
    private extractMethods(code: string): string[] {
        ...
        lines.forEach(line => {
            if (line.includes('public') && (line.includes('(') || line.includes('fn'))) {
