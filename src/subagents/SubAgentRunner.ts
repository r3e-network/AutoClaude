...
export class SubAgentRunner {
  ...
  async runAgentLoop(forceAnalysis: boolean = false): Promise<void> {
    ...
    if (allPassed) {
      ...
      vscode.window.showInformationMessage(
        "Sub-agents are analyzing for improvements",
      );
      return;
    }
    ...
    while (iteration < this.config.maxIterations && !allPassed) {
      ...
      // Show progress
      vscode.window.showInformationMessage(
        `Sub-agent iteration ${iteration}: Waiting for fixes...`,
      );
      ...
      if (allPassed) {
        vscode.window.showInformationMessage(
          `✅ All agent checks passed after ${iteration} iteration(s)!`,
        );
        return;
      }
    }
  ...
  async runMessageWithAgents(message: MessageItem): Promise<void> {
    ...
    vscode.window.showInformationMessage(
      "Processing message with sub-agent support...",
    );
