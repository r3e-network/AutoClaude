...
export class TestFixerAgent extends BaseProductionAgent {
  ...
  async analyzeTestResults(
    ...
    if (error.includes("TypeError")) {
      ...
      suggestedFixes.push(
        "Fix type mismatches and add proper type annotations",
      );
    }
  ...
  async executeSimple(): Promise<{ success: boolean; message: string }> {
    try {
      ...
      return {
        success: false,
        message: `Test analysis failed: ${error instanceof Error ? error.message : String(error)}`,
      };
    }
  }

  private generateFixPlan(analysis: TestAnalysisResult): string {
...
export class TestCreatorAgent extends BaseProductionAgent {
  ...
  private async analyzeTestCoverage(): Promise<TestCoverageAnalysis> {
    try {
      ...
      const uncoveredFiles = srcFiles.filter(
        (src) =>
          !testFiles.some((test) =>
            test.includes(path.basename(src, path.extname(src))),
          ),
      );
...
export class DocGeneratorAgent extends BaseProductionAgent {
  ...
  private async analyzeProject(): Promise<ProjectInfo> {
    ...
    let packageInfo: Partial<ProjectInfo> = {};
  ...
  private async generateReadme(projectInfo: ProjectInfo): Promise<void> {
...
`;
  ...
  private async generateApiDocs(projectInfo: ProjectInfo): Promise<void> {
...
`;
  ...
  private async generateArchitectureDocs(projectInfo: ProjectInfo): Promise<void> {
...
export class DockerCreatorAgent extends BaseProductionAgent {
  ...
  private async analyzeProjectType(): Promise<ProjectTypeInfo> {
    const packageJsonPath = path.join(this.workspaceRoot, "package.json");
  ...
  private async createDockerfile(projectInfo: ProjectTypeInfo): Promise<void> {
    let dockerfile = "";
...
`;
  ...
  private async createDockerCompose(projectInfo: ProjectTypeInfo): Promise<void> {
...
export class CodeCleanerAgent extends BaseProductionAgent {
  ...
  private async analyzeCodeQuality(): Promise<CodeQualityIssues> {
    const issues = {
      deadCode: [] as string[],
      lintErrors: [] as string[],
      outdatedDeps: [] as string[],
      duplicateCode: [] as string[],
      largeFiles: [] as LargeFileInfo[],
      totalIssues: 0,
    };
    ...
    try {
      ...
      // Parse lint errors
      const lintOutput = error && typeof error === 'object' && 'stdout' in error ? (error as { stdout?: string }).stdout : undefined;
      const lintErrors = lintOutput?.match(/\d+ errors?/);
      if (lintErrors) {
  ...
  private generateCleanupPlan(issues: CodeQualityIssues): string {
