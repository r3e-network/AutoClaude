...
export class IntegrationTestingAgent extends SubAgent {
  constructor(workspacePath: string) {
    const config: SubAgentConfig = {
      ...
      description:
        "Creates and manages comprehensive integration tests automatically",
      category: "quality",
      enabled: true,
      icon: "🧪",
      capabilities: [
...
You ensure that all code changes are thoroughly tested before deployment.`,
    ...
    try {
      ...
      try {
        const { stdout: coverageOutput } = await execAsync(
          'npm run test:coverage --silent 2>/dev/null || echo "0"',
          {
            cwd: this.workspacePath,
          },
        );
  ...
  private async identifyMissingTests(): Promise<
  ...
  > {
    ...
    try {
      ...
      if (
        ...
        !testFiles.some((f) => f.includes("error") || f.includes("exception"))
      ) {
        missing.push({
          type: "Error Handling",
          description:
            "No tests found for error scenarios and exception handling",
        });
      }
    } catch (error) {
  ...
  private async findApiEndpoints(): Promise<string[]> {
    ...
    try {
      ...
      const { stdout } = await execAsync(
        'grep -r "app\\." . --include="*.ts" --include="*.js" --include="*.py" | grep -E "(get|post|put|delete|patch)\\(" | head -20',
        {
          cwd: this.workspacePath,
        },
      );
  ...
  private async findDatabaseOperations(): Promise<string[]> {
    ...
    try {
      ...
      const { stdout } = await execAsync(
        'grep -r "\\(save\\|find\\|update\\|delete\\|create\\|insert\\)" . --include="*.ts" --include="*.js" | grep -v test | head -10',
        {
          cwd: this.workspacePath,
        },
      );
  ...
  private async findErrorScenarios(): Promise<string[]> {
    try {
      const { stdout } = await execAsync(
        'grep -r "throw\\|catch\\|error" . --include="*.ts" --include="*.js" | grep -v test | wc -l',
        {
          cwd: this.workspacePath,
        },
      );
  ...
  private async suggestTestImprovements(): Promise<
  ...
  > {
    ...
    if (testFiles.length > 0) {
      try {
        ...
        const hasTestFolder =
          fs.existsSync(path.join(this.workspacePath, "test")) ||
        ...
        const hasTestUtils = testFiles.some(
        ...
        const hasFixtures =
          fs.existsSync(path.join(this.workspacePath, "fixtures")) ||
