...
export class CodeUnderstandingAgent extends SubAgent {
  ...
  constructor(workspacePath: string) {
    const config: SubAgentConfig = {
      ...
      description:
        "Deeply analyzes code patterns, structures, and semantics to guide development",
      category: "analysis",
      enabled: true,
      icon: "🔍",
      capabilities: [
        {
          ...
          description:
            "Identify design patterns, coding conventions, and architectural styles",
          action: "analyze",
        },
        {
...
You help Claude write code that fits naturally with existing patterns and maintains consistency.`,
  ...
  async analyzeResults(context: SubAgentContext): Promise<string> {
    ...
    if (patterns.length > 0) {
      ...
      patterns.forEach((pattern) => {
        analysis.push(
          `- ${pattern.name}: ${pattern.description} (${pattern.frequency} occurrences)`,
        );
      });
      analysis.push("");
    }
    ...
    if (refactoring.length > 0) {
      ...
      refactoring.forEach((opp) => {
        analysis.push(
          `- ${opp.type}: ${opp.description} (Impact: ${opp.impact})`,
        );
      });
      analysis.push("");
    }
  ...
  private async analyzeCodebaseStructure(): Promise<any> {
    ...
    try {
      ...
      for (const { ext, lang } of extensions) {
        try {
          const { stdout } = await execAsync(
            `find . -name "*.${ext}" -not -path "./node_modules/*" -not -path "./.git/*" | xargs wc -l | tail -1`,
            {
      ...
      structure.complexityScore = Math.min(
        Math.floor(structure.totalLines / 1000) + structure.languages.length,
        10,
      );
    } catch (error) {
  ...
  private async identifyCodePatterns(): Promise<
    Array<{ name: string; description: string; frequency: number }>
  > {
  ...
  private async findRefactoringOpportunities(): Promise<
