...
export class TestAgent extends SubAgent {
  constructor(workspacePath: string) {
    const config: SubAgentConfig = {
      ...
      name: "Test Agent",
      description:
        "Manages test suites, ensures coverage, and maintains test quality",
      category: "quality",
      enabled: true,
      icon: "✅",
      capabilities: [
...
Focus on creating fast, reliable, and maintainable tests that catch real bugs.`,
  ...
  async analyzeResults(context: SubAgentContext): Promise<string> {
    ...
    const failedTests = context.scriptResult.errors.filter(
      (e) => e.includes("FAIL") || e.includes("failed") || e.includes("Failed"),
    ).length;
    const timeouts = context.scriptResult.errors.filter(
      (e) => e.includes("timeout") || e.includes("Timeout"),
    ).length;
    const assertionErrors = context.scriptResult.errors.filter(
      (e) =>
        e.includes("assert") || e.includes("expect") || e.includes("Expected"),
    ).length;

    analysis.push(
      `Test suite failed with ${context.scriptResult.errors.length} issues:`,
    );
    analysis.push("");
    ...
    analysis.push(
      "1. Address timeout issues first (may indicate infinite loops)",
    );
