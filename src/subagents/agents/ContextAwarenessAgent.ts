...
export class ContextAwarenessAgent extends SubAgent {
  ...
  constructor(workspacePath: string) {
    const config: SubAgentConfig = {
      ...
      description:
        "Understands project structure, dependencies, and relationships between files",
      category: "analysis",
      enabled: true,
      icon: "🧠",
      capabilities: [
        {
          ...
          description:
            "Map out entire project architecture and file relationships",
          action: "analyze",
        },
        {
          ...
          description:
            "Identify all dependencies and imports for a given file or feature",
          action: "analyze",
        },
        {
        ...
        },
        {
          ...
          description:
            "Create comprehensive context for Claude to understand the task",
          action: "generate",
        },
      ],
    ...
    for (const pattern of filePatterns) {
      ...
      for (const match of matches) {
        if (match[1]) {
          ...
          try {
            const { stdout } = await execAsync(
              `find . -name "*${match[1]}*" -type f | head -10`,
              {
                cwd: this.workspacePath,
              },
            );
