...
export class FormatAgent extends SubAgent {
  constructor(workspacePath: string) {
    const config: SubAgentConfig = {
      ...
      description:
        "Ensures consistent code formatting and style across the project",
      category: "quality",
      enabled: true,
      icon: "💅",
      capabilities: [
...
Focus on making code beautiful, consistent, and easy to read.`,
  ...
  async analyzeResults(context: SubAgentContext): Promise<string> {
    ...
    analysis.push(
      `Found formatting issues in ${filesWithIssues || "multiple"} files:`,
    );
    analysis.push("");

    // Group errors by type if possible
    const indentationIssues = context.scriptResult.errors.filter(
      (e) => e.includes("indent") || e.includes("space") || e.includes("tab"),
    ).length;
    const lineIssues = context.scriptResult.errors.filter(
      (e) =>
        e.includes("line") || e.includes("newline") || e.includes("length"),
    ).length;
    const importIssues = context.scriptResult.errors.filter(
      (e) => e.includes("import") || e.includes("require"),
    ).length;
