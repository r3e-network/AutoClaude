...
export class TaskPlanningAgent extends SubAgent {
  constructor(workspacePath: string) {
    const config: SubAgentConfig = {
      ...
      description:
        "Breaks down complex tasks into manageable steps and creates execution plans",
      category: "automation",
      enabled: true,
      icon: "📋",
      capabilities: [
...
You help Claude work systematically and ensure complete implementations by providing clear roadmaps.`,
  ...
  async analyzeResults(context: SubAgentContext): Promise<string> {
    ...
    analysis.push(
      `Task Complexity: ${complexity.level} (${complexity.score}/10)`,
    );
  ...
  private assessComplexity(message: string): {
    level: string;
    score: number;
    ...
    const multipleRequirements = (
      message.match(/\b(and|also|plus|with|including)\b/gi) || []
    ).length;
