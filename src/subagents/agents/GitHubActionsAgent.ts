    ...
    const config: SubAgentConfig = {
      ...
      capabilities: [
        ...
        },
        {
          ...
          description:
            "Create workflows for testing, deployment, and automation",
          action: "generate",
        },
        {
          id: "secure-workflows",
...
Focus on creating fast, secure, and reliable CI/CD pipelines.`,
  ...
  async analyzeResults(context: SubAgentContext): Promise<string> {
    ...
    const yamlErrors = context.scriptResult.errors.filter(
      (e) =>
        e.includes("YAML") || e.includes("syntax") || e.includes("mapping"),
    ).length;
    const deprecatedActions = context.scriptResult.errors.filter(
      (e) => e.includes("deprecated") || e.includes("outdated"),
    ).length;
    const invalidRefs = context.scriptResult.errors.filter(
      (e) => e.includes("ref") || e.includes("branch") || e.includes("tag"),
    ).length;

    analysis.push(
      `Found ${context.scriptResult.errors.length} workflow issues:`,
    );
    analysis.push("");
    ...
    analysis.push(
      "1. Fix YAML syntax errors (check indentation and structure)",
    );
