...
export class SecurityAuditAgent extends SubAgent {
  constructor(workspacePath: string) {
    const config: SubAgentConfig = {
      ...
      description:
        "Identifies and fixes security vulnerabilities automatically",
      category: "security",
      enabled: true,
      icon: "🔒",
      capabilities: [
        ...
        },
        {
          ...
          description:
            "Detect SQL injection, XSS, and other injection vulnerabilities",
          action: "analyze",
        },
        {
...
You help Claude build secure applications that protect against common vulnerabilities and attacks.`,
  ...
  private async findHardcodedSecrets(): Promise<
  ...
  > {
    ...
    try {
      const secretPatterns = [
        ...
        {
          type: "JWT Secret",
          patterns: [/jwt[_-]?secret\s*[=:]\s*["'][^"']{20,}["']/gi],
        },
        {
          type: "Database URL",
          patterns: [
      ...
      const { stdout } = await execAsync(
        'find . -name "*.ts" -o -name "*.js" -o -name "*.py" | grep -v node_modules | head -20',
        {
  ...
  private async findInjectionVulnerabilities(): Promise<
    Array<{ type: string; description: string; file: string }>
  > {
    ...
    try {
      ...
      const vulnPatterns = [
        ...
        },
        {
          ...
          description:
            "Potential command injection from unsafe command execution",
        },
        {
          type: "Path Traversal",
          patterns: [
  ...
  private async checkAuthenticationSecurity(): Promise<
