...
You help Claude create high-performance applications that scale efficiently.`,
  ...
  async analyzeResults(context: SubAgentContext): Promise<string> {
    ...
    analysis.push(
      `Cyclomatic Complexity: ${complexity.average.toFixed(1)} (${complexity.level})`,
    );
    analysis.push(
    ...
    if (antiPatterns.length > 0) {
      ...
      antiPatterns.forEach((pattern) => {
        analysis.push(
          `- ${pattern.type}: ${pattern.description} (${pattern.occurrences} occurrences)`,
        );
      });
      analysis.push("");
    }
    ...
    if (bundleInfo.hasBundle) {
      ...
      analysis.push(
        `Large Dependencies: ${bundleInfo.largeDependencies.join(", ")}`,
      );
      analysis.push("");
    }
  ...
  private async analyzeCodeComplexity(): Promise<any> {
    ...
    try {
      ...
      const { stdout } = await execAsync(
        'find . -name "*.ts" -o -name "*.js" | grep -v node_modules | head -20',
        {
  ...
  private async findPerformanceAntiPatterns(): Promise<
    Array<{ type: string; description: string; occurrences: number }>
  > {
    ...
    try {
      const { stdout } = await execAsync(
        'find . -name "*.ts" -o -name "*.js" | grep -v node_modules | head -10',
        {
          cwd: this.workspacePath,
        },
      );
      ...
      const patterns = [
        ...
        },
        {
          ...
          description:
            "Blocking file operations that can freeze the event loop",
        },
        {
          type: "Large Array Operations",
          regex: /\.map\s*\([^)]*\)\.filter\s*\([^)]*\)/g,
          description:
            "Chained array operations that create intermediate arrays",
        },
        {
        ...
        },
        {
          ...
          description:
            "Promise chains that could be simplified with async/await",
        },
      ];
  ...
  private async findDatabaseIssues(): Promise<
  ...
  > {
    ...
    try {
      const { stdout } = await execAsync(
        'find . -name "*.ts" -o -name "*.js" -o -name "*.sql" | grep -v node_modules | head -20',
        {
          cwd: this.workspacePath,
        },
      );
        ...
        try {
          ...
          if (content.match(/SELECT.*FROM.*(?!WHERE)/i)) {
            issues.push({
              ...
              description:
                "SELECT queries without WHERE clauses may return too much data",
            });
          }
    ...
    return [...new Set(issues.map((i) => JSON.stringify(i)))].map((i) =>
      JSON.parse(i),
    ); // Remove duplicates
  }
}
