...
export abstract class SubAgent implements SubAgentExecutor {
  ...
  async runCheck(): Promise<ScriptResult> {
    ...
    try {
      ...
      const scriptError = error as { stdout?: string; message?: string };
      if (scriptError.stdout) {
        try {
          const result = JSON.parse(scriptError.stdout as string);
          if (typeof result.passed === "boolean") {
            return result;
          }
        } catch {
      ...
      debugLog(`Script execution error: ${scriptError.message || String(error)}`);
      return {
        passed: false,
  ...
  async execute(request: SubAgentRequest): Promise<SubAgentResponse> {
    debugLog(
      `🤖 SubAgent ${this.config.name} executing action: ${request.action}`,
    );
  ...
  protected buildClaudeMessage(request: SubAgentRequest): string {
    ...
    if (request.context.scriptResult) {
      ...
      if (request.context.scriptResult.passed) {
        ...
        if (
          request.context.scriptResult.warnings &&
          request.context.scriptResult.warnings.length > 0
        ) {
          parts.push("");
    ...
    if (request.context.iterationCount > 0) {
      parts.push("");
      parts.push(
        `Note: This is iteration ${request.context.iterationCount} of ${request.context.maxIterations}`,
      );
    }
  ...
  protected getSuggestedNextActions(
    currentAction: SubAgentAction,
  ): SubAgentAction[] {
