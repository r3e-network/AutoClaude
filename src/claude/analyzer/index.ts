...
export function analyzeClaudeOutput(output?: string): TaskAnalysis {
  ...
  if (
    analysis.unfinishedTodos.length > 0 ||
    analysis.incompleteSteps.length > 0
  ) {
    ...
    analysis.hasUnfinishedTasks = true;
    analysis.confidence = "medium";
    analysis.incompleteSteps.push(
      "Detected planned actions that may not have been completed",
    );
  ...
  debugLog(
    `📊 Analysis complete: ${analysis.hasUnfinishedTasks ? "Found unfinished tasks" : "No unfinished tasks detected"}`,
  );
...
function generateContinuationMessage(analysis: TaskAnalysis): string {
  ...
  if (analysis.confidence === "high") {
    parts.push(
      "I noticed you have unfinished tasks. Please continue with the previous work.",
    );
  } else if (analysis.confidence === "medium") {
    parts.push(
      "It appears there may be incomplete work from the previous task. Please review and continue if needed.",
    );
  } else {
    parts.push(
      "The previous session may have ended with incomplete work. Please check if there's anything left to finish.",
    );
  }
  ...
  if (analysis.incompleteSteps.length > 0) {
    if (analysis.incompleteSteps.some((step) => step.startsWith("Continue:"))) {
      ...
      analysis.incompleteSteps.some((step) => step.startsWith("Task:"))
    ) {
...
export async function checkAndResumeTasks(): Promise<boolean> {
  ...
  const aggressiveMode = config.get<boolean>(
    "session.aggressiveTaskDetection",
    false,
  );
...
function checkAndResumeTasksWithAnalysis(analysis: TaskAnalysis): boolean {
  debugLog(
    `🔄 Automatically resuming unfinished tasks (confidence: ${analysis.confidence})`,
  );
...
function analyzeClaudeOutputAggressive(): TaskAnalysis {
  ...
  const hasAggressiveMatch = aggressivePatterns.some((pattern) =>
    pattern.test(screenContent),
  );
  ...
  if (hasAggressiveMatch || hasClaudeStoppedMidTask()) {
    ...
    analysis.confidence = "low";
    analysis.incompleteSteps.push(
      "Aggressive detection found potential incomplete work",
    );
    analysis.suggestedContinuation = generateContinuationMessage(analysis);
  }
...
export function hasClaudeStoppedMidTask(): boolean {
  ...
  return lastFewLines.some((line) => {
    ...
    return incompleteSentencePatterns.some((pattern) =>
