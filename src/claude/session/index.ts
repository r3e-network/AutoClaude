...
} from "../../services/dependency-check";
...
export async function startClaudeSession(
  ...
  try {
    ...
    if (!wrapperPath) {
      const errorMsg =
        "Claude PTY wrapper not found. Please reinstall the extension.";
    ...
    spawnedProcess.stdout?.on("data", (data: Buffer) => {
      ...
      if (
        ...
        if (currentMessage && isCurrentUsageLimit(output)) {
          ...
          debugLog(
            "⚠️ Skipped: Usage limit is old or not within 6-hour window",
          );
        }
        return;
      }
      ...
      const isAuthError = claudeAuthErrors.some((authError) =>
      ...
      if (hasPermissionPrompt && !sessionReady) {
        debugLog(
          "🔐 Permission prompt detected during startup - session ready for user interaction",
        );
        ...
        updateSessionState();
        vscode.window.showInformationMessage(
          "Claude session started and ready! You can now process the message queue.",
        );
      }
    });
    ...
    spawnedProcess.on("close", (code: number | null) => {
      ...
      const closeMessage = formatTerminalOutput(
      ...
      if (currentMessage && currentMessage.status === "processing") {
        debugLog(
          `❌ Current message #${currentMessage.id} marked as error due to process closure`,
        );
        currentMessage.status = "error";
      ...
      if (wasProcessing) {
        vscode.window.showWarningMessage(
          "Claude process closed unexpectedly while processing. You can restart the session.",
        );
      } else {
    ...
    spawnedProcess.on("error", (error: Error) => {
      ...
      const errorMessage = formatTerminalOutput(
      ...
      if (currentMessage && currentMessage.status === "processing") {
        debugLog(
          `❌ Current message #${currentMessage.id} marked as error due to process error`,
        );
        currentMessage.status = "error";
      ...
      if (wasProcessing) {
        vscode.window.showErrorMessage(
          `Claude process error while processing: ${error.message}`,
        );
      } else {
        vscode.window.showErrorMessage(
          `Claude process error: ${error.message}`,
...
export function resetClaudeSession(): void {
  ...
  import("../output")
    .then(({ clearClaudeOutput, flushClaudeOutput }) => {
      flushClaudeOutput();
      clearClaudeOutput();
    })
    .catch((error) => {
  ...
  updateSessionState();
  vscode.window.showInformationMessage(
    "Claude session reset. You can now start a new session.",
  );
}
...
export function handleClaudeKeypress(key: string): void {
  if (!claudeProcess || !claudeProcess.stdin) {
    ...
    vscode.window.showWarningMessage(
      "Claude process not available for keypress input",
    );
    return;
  }
