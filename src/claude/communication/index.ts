...
import {
  ...
  ensureHistoryRun,
  updateMessageStatusInHistory,
} from "../../queue/processor/history";
import { TIMEOUT_MS, ANSI_CLEAR_SCREEN_PATTERNS } from "../../core/constants";
import {
  startClaudeSession,
  isClaudeProcessHealthy,
} from "../../claude/session";
...
export async function processNextMessage(): Promise<void> {
  ...
  if (messageQueue.length === 0) {
    debugLog(
      "Queue empty - waiting for new messages (processing remains active)",
    );
    updateWebviewContent();
    updateSessionState();
    vscode.window.showInformationMessage(
      "All messages processed. Claude session remains active. Add messages to continue.",
    );
  ...
  if (!message) {
    debugLog(
      "No pending messages found - processing remains active for new messages",
    );
    updateWebviewContent();
    updateSessionState();
    vscode.window.showInformationMessage(
      "No pending messages to process. Processing remains active.",
    );
    return;
  }
  ...
  if (!isClaudeProcessHealthy()) {
    ...
    vscode.window.showInformationMessage(
      "Claude session disconnected. Restarting...",
    );
    ...
    try {
      ...
      vscode.window.showErrorMessage(
        `Failed to restart Claude session: ${error instanceof Error ? error.message : String(error)}`,
      );
  ...
  if (!sessionReady) {
    ...
    vscode.window.showWarningMessage(
      "Claude session not ready. Please wait for Claude to be ready first.",
    );
    setProcessingQueue(false);
    return;
  }
  ...
  debugLog(
    `📋 Processing message #${message.id}: ${message.text.substring(0, 50)}...`,
  );
  ...
  try {
    ...
    if (message.attachedScripts && message.attachedScripts.length > 0) {
      try {
        ...
        debugLog(
          `⚠️ Script execution failed for message #${message.id}: ${scriptError}`,
        );
    ...
    const autoResumeEnabled = config.get<boolean>(
    ...
    if (autoResumeEnabled) {
      ...
      debugLog(
        `⏳ Waiting ${resumeDelay}ms before checking for unfinished tasks...`,
      );
      ...
      if (hasResumed) {
        debugLog(
          "📝 Automatically added continuation message for unfinished tasks",
        );
        vscode.window.showInformationMessage(
          "Detected unfinished tasks - automatically continuing...",
        );
      }
    } else {
...
export async function sendMessageToClaudeProcess(
  message: MessageItem,
  retryCount: number = 0,
): Promise<void> {
  const maxRetries = 3;
  ...
  if (!claudeProcess || !claudeProcess.stdin) {
    if (retryCount < maxRetries) {
      debugLog(
        `❌ Claude process not available, attempting to restart (retry ${retryCount + 1}/${maxRetries})`,
      );
      ...
      throw new Error(
        `Claude process not available after ${maxRetries} retries`,
      );
    }
  }
  ...
  try {
    ...
    for (let i = 0; i < messageBuffer.length; i += CHUNK_SIZE) {
      const chunk = messageBuffer.subarray(
        i,
    ...
    if (retryCount < maxRetries) {
      debugLog(
        `🔄 Retrying message send (attempt ${retryCount + 1}/${maxRetries})`,
      );
      await new Promise((resolve) => setTimeout(resolve, 1000));
      return sendMessageToClaudeProcess(message, retryCount + 1);
    } else {
      throw new Error(
        `Failed to send message after ${maxRetries} retries: ${error}`,
      );
    }
  }
}
...
function waitForPrompt(): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      debugLog(
        `❌ Timeout after ${TIMEOUT_MS / 1000}s waiting for Claude to be ready`,
      );
      cleanup();
    ...
    const analyzeCurrentScreen = () => {
      ...
      debugLog(
        `🔍 Analyzing screen (${currentScreenBuffer.length} chars, ${timeSinceLastOutput}ms since last output)`,
      );
      ...
      if (hasPermissionPrompt && !waitingForPermission) {
        ...
        vscode.window.showInformationMessage(
          "Claude is asking for permission. Use the Claude output area to navigate and select your choice.",
        );
        screenAnalysisTimer = setTimeout(
          analyzeCurrentScreen,
          SCREEN_ANALYSIS_INTERVAL_MS,
      ...
      if (!isReady && timeSinceLastOutput >= EXTENDED_STABILITY_MS) {
        ...
        if (hasClaudeStoppedMidTask()) {
          debugLog(
            `⚠️ Claude appears to have stopped mid-task after ${timeSinceLastOutput}ms`,
          );
          cleanup();
      ...
      if (isReady && timeSinceLastOutput >= DEBOUNCE_THRESHOLD_MS) {
        debugLog(
          `✅ Claude is ready! Pattern detected and ${timeSinceLastOutput}ms of stability`,
        );
        cleanup();
        resolve();
      } else if (isReady) {
        debugLog(
          `⏳ Ready pattern detected but waiting for stability (${timeSinceLastOutput}ms < ${DEBOUNCE_THRESHOLD_MS}ms)`,
        );
        screenAnalysisTimer = setTimeout(
          analyzeCurrentScreen,
          SCREEN_ANALYSIS_INTERVAL_MS,
...
export async function startProcessingQueue(
  ...
  debugLog(
    `🚀 startProcessingQueue called with skipPermissions=${skipPermissions}`,
  );
  ...
  debugLog(
    `🔄 Setting processingQueue to true, current queue length: ${messageQueue.length}`,
  );
  setProcessingQueue(true);
  updateSessionState();
  ...
  if (messageQueue.length === 0) {
    debugLog(
      "📭 Queue is empty after starting processing - waiting for messages",
    );
    vscode.window.showInformationMessage(
      "Claude session is ready. Add messages to start processing.",
    );
    return;
  }
...
export function stopProcessingQueue(): void {
  ...
  updateSessionState();
  vscode.window.showInformationMessage(
    "Processing stopped. Claude session remains active.",
  );
}
...
async function runAttachedScripts(message: MessageItem): Promise<void> {
  ...
  debugLog(
    `🔧 Running ${message.attachedScripts.length} attached scripts sequentially for message #${message.id}`,
  );
  ...
  try {
    ...
    for (const scriptId of message.attachedScripts) {
      ...
      try {
        ...
        if (!result) {
          ...
          failureDetails = [
            `Script ${scriptName} not found or failed to execute`,
          ];
        ...
        if (!result.passed) {
          ...
          if (result.errors.length > MAX_ERROR_LINES) {
            ...
            limitedErrors.push(
              `... and ${result.errors.length - MAX_ERROR_LINES} more errors`,
            );
          }
          ...
          debugLog(
            `   First ${Math.min(MAX_ERROR_LINES, result.errors.length)} errors:`,
          );
        ...
        failedScriptName = scriptName;
        failureDetails = [
          `Error running script: ${scriptError instanceof Error ? scriptError.message : String(scriptError)}`,
        ];
    ...
    if (failedScript) {
        ...
        .then((selection) => {
          if (selection === "Show Details") {
            ...
            const outputChannel = vscode.window.createOutputChannel(
              "AutoClaude Script Results",
            );
            outputChannel.clear();
            outputChannel.appendLine(
              `Script Check Failed: ${failedScriptName}`,
            );
      ...
      const skippedScripts = message.attachedScripts.slice(
        message.attachedScripts.indexOf(failedScript) + 1,
      );
      if (skippedScripts.length > 0) {
        const skippedNames = skippedScripts
          .map((id) => {
      ...
      vscode.window.showInformationMessage(
        `✅ All attached scripts passed: ${scriptNames}`,
      );
      debugLog(
        `✅ All ${message.attachedScripts.length} scripts passed successfully`,
      );
    }
  } catch (error) {
    debugLog(`❌ Error initializing script runner: ${error}`);
    vscode.window.showErrorMessage(
      `Failed to run attached scripts: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
}
