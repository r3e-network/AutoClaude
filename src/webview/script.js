  ...
  for (const pattern of dangerousPatterns) {
    if (pattern.test(text)) {
      return {
        valid: false,
        error:
          "Message contains potentially dangerous content. Enable XSS bypass in settings if needed.",
      };
    }
  }
...
function clearClaudeOutput() {
  try {
    ...
    if (claudeOutput) {
      ...
      const messageSpan = createSafeElement(
        "span",
        "Output cleared - ready for new Claude output...",
        "",
      );
...
function renderQueue() {
  try {
    ...
    if (messageQueue.length === 0) {
      container.innerHTML = "";
      const emptyMessage = createSafeElement(
        "div",
    ...
    messageQueue.forEach((item, index) => {
      ...
      if (item.status === "waiting" && item.waitSeconds > 0) {
        ...
        const countdownDiv = createSafeElement(
          "div",
          `Resuming in ${hours}h ${minutes}m ${seconds}s`,
          "countdown",
        );
      ...
      if (item.status === "completed" && item.output) {
        const outputDiv = createSafeElement(
          "div",
          item.output,
          "queue-item-output",
        );
      ...
      if (item.status === "error" && item.error) {
        const errorDiv = createSafeElement(
          "div",
          `Error: ${item.error}`,
          "queue-item-error",
        );
    ...
    const container = document.getElementById("queueContainer");
    container.innerHTML = "";
    const errorMessage = createSafeElement(
      "div",
      "Error rendering queue",
      "error-message",
    );
...
function updateButtonStates() {
  ...
  console.log("Start button state:", {
    ...
    reason: !sessionState.isSessionRunning
      ? "no session"
      : sessionState.wasStopped
        ? "manually stopped"
  ...
  resetBtn.disabled =
    !sessionState.isSessionRunning || sessionState.isProcessing;
...
function renderHistory(history) {
  try {
    ...
    if (!history || history.length === 0) {
      ...
      const emptyMessage = createSafeElement(
        "div",
        "No previous runs found for this workspace",
        "empty-history",
      );
      container.appendChild(emptyMessage);
      return;
    ...
    history.forEach((run) => {
      ...
      const duration = run.endTime
        ? Math.round(
            (new Date(run.endTime) - new Date(run.startTime)) / 1000 / 60,
          ) + " min"
        : "Ongoing";
      ...
      const timeDiv = createSafeElement(
        "div",
        `${startTime} (${duration})`,
        "history-item-time",
      );
      ...
      const totalStat = createSafeElement(
        "div",
        `📊 Total: ${run.totalMessages}`,
        "history-stat history-stat-total",
      );
      const completedStat = createSafeElement(
        "div",
        `✅ Completed: ${run.completedMessages}`,
        "history-stat history-stat-completed",
      );
      const errorStat = createSafeElement(
        "div",
        `❌ Errors: ${run.errorMessages}`,
        "history-stat history-stat-errors",
      );
      const waitingStat = createSafeElement(
        "div",
        `⏳ Waiting: ${run.waitingMessages}`,
        "history-stat history-stat-waiting",
      );
      ...
      run.messages.forEach((msg) => {
        ...
        const statusSpan = createSafeElement(
          "span",
          msg.status.toUpperCase(),
          `status-${msg.status}`,
        );
        const timeSpan = createSafeElement(
          "span",
...
window.addEventListener("message", (event) => {
  ...
  switch (message.command) {
    ...
    case "setSecuritySettings":
      ...
      if (securityWarning) {
        securityWarning.style.display = message.allowDangerousXssbypass
          ? "block"
          : "none";
      }
      break;
...
function appendToTerminal(output) {
  try {
    ...
    const readyMessage = terminalOutput.querySelector(
    ...
    outputElement.style.cssText =
      "white-space: pre; word-wrap: break-word; line-height: 1.4; font-family: inherit;";
  ...
  try {
    ...
    if (timeSinceLastRender >= CLAUDE_RENDER_THROTTLE_MS) {
      ...
      if (!claudeRenderTimer) {
        ...
        console.log(
          "🔄 Claude render already scheduled, updating pending output",
        );
      }
    }
  } catch (error) {
...
function performClaudeRender(output) {
  try {
    ...
    claudeOutput.style.boxShadow =
      "0 0 20px rgba(0, 255, 136, 0.5), inset 0 1px 0 rgba(255, 255, 255, 0.1)";
    setTimeout(() => {
      claudeOutput.style.borderColor = "#4a9eff";
      claudeOutput.style.boxShadow =
        "0 0 20px rgba(74, 158, 255, 0.3), inset 0 1px 0 rgba(255, 255, 255, 0.1)";
    }, 800);
  } catch (error) {
...
function showFileAutocomplete(textarea, atPosition) {
  ...
  let autocompleteContainer = document.getElementById(
    "fileAutocompleteContainer",
  );
  ...
  window.addEventListener("scroll", fileAutocompleteState.pageScrollHandler, {
    passive: true,
  });
  document.addEventListener("scroll", fileAutocompleteState.pageScrollHandler, {
    passive: true,
  });
  ...
  autocompleteContainer.innerHTML =
    '<div class="file-autocomplete-loading">Loading files...</div>';
...
function renderFileAutocomplete(files, pagination = null) {
  ...
  if (fileAutocompleteState.currentPage === 0) {
    ...
    if (pagination && pagination.hasNextPage) {
      html +=
        '<div class="file-autocomplete-loading-more" id="loadingMore">Loading more files...</div>';
    }
    ...
    files.forEach((file, index) => {
      const globalIndex =
        fileAutocompleteState.files.length - files.length + index;
  ...
  if (fileAutocompleteState.currentPage === 0) {
    autocompleteContainer
      .querySelectorAll(".file-autocomplete-item")
      .forEach((item, index) => {
...
function loadMoreFiles() {
  if (
    fileAutocompleteState.pagination &&
...
function scrollSelectedItemIntoView() {
  ...
  const selectedItem = document.getElementById(
    `file-item-${fileAutocompleteState.selectedIndex}`,
  );
...
function selectFileReference(index) {
  if (
    !fileAutocompleteState.isOpen ||
    index >= fileAutocompleteState.files.length
  ) {
    return;
  }
  ...
  const afterQuery = currentValue.substring(
    fileAutocompleteState.atPosition + 1 + fileAutocompleteState.query.length,
  );
...
function initializeButtonEventListeners() {
  ...
  // Debug buttons
  const simulateUsageLimitBtn = document.getElementById(
    "simulateUsageLimitBtn",
  );
  if (simulateUsageLimitBtn)
    simulateUsageLimitBtn.addEventListener("click", simulateUsageLimit);
  ...
  if (debugQueueStateBtn)
    debugQueueStateBtn.addEventListener("click", debugQueueState);
  ...
  if (clearClaudeOutputBtn)
    clearClaudeOutputBtn.addEventListener("click", clearClaudeOutput);

  // Script buttons
  const runScriptChecksBtn = document.getElementById("runScriptChecksBtn");
  if (runScriptChecksBtn)
    runScriptChecksBtn.addEventListener("click", runScriptChecks);

  const runScriptLoopBtn = document.getElementById("runScriptLoopBtn");
  if (runScriptLoopBtn)
    runScriptLoopBtn.addEventListener("click", runScriptLoop);
...
function handleScriptDrop(e) {
  ...
  if (draggedScriptElement !== this) {
    ...
    const allScripts = Array.from(
      scriptsConfig.querySelectorAll(".script-item"),
    );
...
function updateScriptOrder() {
  ...
  scriptOrder = Array.from(scriptItems).map((item) =>
    item.getAttribute("data-script-id"),
  );
...
function runSingleScript(scriptId) {
  ...
  if (scriptResultsSection) {
    ...
    scriptResultsSection.innerHTML =
      '<div style="padding: 10px;">Running ' + scriptId + "...</div>";
  }
...
function runMessageInLoop(messageId) {
  ...
  const enabledScripts = scriptOrder.filter(
    (scriptId) => document.getElementById(`script-${scriptId}`).checked,
  );

  if (enabledScripts.length === 0) {
    showError(
      "Please enable at least one script check before running message in loop",
    );
    return;
  }
...
function showScriptSuggestions(query, atPosition) {
  const suggestions = availableScripts.filter(
    (script) =>
...
function getScriptIcon(script) {
  if (
    script.id.includes("automation") ||
    script.id.includes("ai-") ||
    script.id.includes("tdd-") ||
    script.id.includes("doc-")
  ) {
    return "🤖";
  }
  return "🔍";
...
function updateSubAgentDisplay(subAgentData) {
  ...
  subAgentData.agents.forEach((agent) => {
    ...
    if (scriptItem) {
      ...
      const capabilitiesDiv = scriptItem.querySelector(
