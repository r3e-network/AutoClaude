...
export class SessionRecoveryManager extends EventEmitter {
  private session: typeof sessionAdapter | null = null;
  ...
  async initializeSession(
    ...
    try {
      ...
      this.logger.info(
        "Session initialized successfully with health monitoring",
      );
  ...
  private setupSessionHandlers(): void {
    ...
    this.session.on("exit", ({ exitCode, signal }) => {
      this.logger.warn(
        `Session exited unexpectedly (code: ${exitCode}, signal: ${signal})`,
      );
    ...
    this.healthMonitor.on("sessionUnhealthy", (details) => {
      ...
      if (this.options.autoRecover) {
        this.initiateRecovery(
          "Session unhealthy - multiple health check failures",
        );
      }
    });
  ...
  async initiateRecovery(reason: string): Promise<void> {
    ...
    this.emit("recoveryStarted", {
      reason,
      attempt: this.recoveryState.retryCount + 1,
    });
    ...
    try {
      ...
      if (this.recoveryState.retryCount >= this.options.maxRetries) {
        throw new Error(
          `Max recovery attempts (${this.options.maxRetries}) exceeded`,
        );
      }
