...
export function removeMessageFromQueue(messageId: string): void {
  try {
    ...
    if (index >= 0) {
      ...
      debugLog(
        `✅ Message removed from queue: ${removedMessage.text.substring(0, 50)}...`,
      );
      vscode.window.showInformationMessage(
        `Message removed: ${removedMessage.text.substring(0, 30)}...`,
      );
    } else {
...
export function duplicateMessageInQueue(messageId: string): void {
  ...
  if (message) {
    ...
    vscode.window.showInformationMessage(
      `Message duplicated: ${message.text.substring(0, 50)}...`,
    );
  }
}
...
export function editMessageInQueue(messageId: string, newText: string): void {
  try {
    debugLog(
      `EditMessageInQueue called with messageId: ${messageId}, newText length: ${newText?.length || 0}`,
    );
...
export function reorderQueue(fromIndex: number, toIndex: number): void {
  if (
    fromIndex < 0 ||
    fromIndex >= messageQueue.length ||
    toIndex < 0 ||
    toIndex >= messageQueue.length
  ) {
    return;
  }
...
export function addMessageToQueueFromWebview(
  message: string,
  ...
  const messageItem: MessageItem = {
    ...
    status: "pending",
    attachedScripts:
      attachedScripts && attachedScripts.length > 0
        ? attachedScripts
        : undefined,
  };
  ...
  if (hasWaitingMessages) {
    vscode.window.showInformationMessage(
      `Message added to queue (waiting for usage limit to reset): ${message.substring(0, 50)}...`,
    );
  } else {
    vscode.window.showInformationMessage(
      `Message added to queue: ${message.substring(0, 50)}...`,
    );
  }
...
function tryAutoStartProcessing(): void {
  ...
  debugLog(
    `🔍 Auto-start check: sessionReady=${sessionReady}, processingQueue=${processingQueue}, hasProcessing=${hasProcessingMessages}, hasPending=${hasPendingMessages}, hasWaiting=${hasWaitingMessages}`,
  );
  ...
  if (shouldAutoStart) {
    if (!processingQueue) {
      debugLog(
        "🚀 Auto-enabling processing for first message in ready session",
      );
