 ...
 */
export async function addMessageWithAutomation(
  message: string,
  currentFile?: string,
): Promise<void> {
  ...
  try {
    ...
    const enhancedMessage = await automationManager.processMessage(
      message,
      currentFile,
    );
 ...
 */
export async function handleErrorWithAutomation(
  error: string,
  context?: any,
): Promise<boolean> {
  if (!automationManager) {
