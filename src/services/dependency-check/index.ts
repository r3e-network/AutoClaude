...
async function verifyPythonVersion(
  ...
  try {
    const result = await checkCommand(pythonCommand, [
      "-c",
...
export async function checkPtyWrapperFile(): Promise<DependencyCheckResult> {
  try {
    ...
    for (const wrapperPath of possiblePaths) {
      ...
      try {
        if (fs.existsSync(wrapperPath)) {
          ...
          debugLog("PTY wrapper not found at:", wrapperPath);
        }
      } catch (error) {
        debugLog("Error checking PTY wrapper path:", wrapperPath, error);
        // Try next path
        continue;
      }
    }
    ...
    return {
      ...
      installInstructions:
        "The extension may not be properly installed. Try reinstalling the extension.",
    };
  } catch (error) {
    return {
      available: false,
      ...
      installInstructions:
        "Please check file permissions and try reinstalling the extension.",
    };
  }
}
...
export function showDependencyStatus(
  results: Awaited<ReturnType<typeof runDependencyCheck>>,
): void {
...
function showInstallationInstructions(
  ...
  instructions +=
    "After installing dependencies, restart VS Code and try again.";
  ...
  vscode.workspace
    ...
    .then(
      ...
      (error) => {
        vscode.window.showErrorMessage(
          `Failed to show installation instructions: ${error instanceof Error ? error.message : String(error)}`,
        );
