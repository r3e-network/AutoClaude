...
export function startScheduledSession(callback: () => void): void {
  ...
  try {
    ...
    debugLog(
      `🕒 Scheduling Claude session start for ${config.session.scheduledStartTime} (${formatRelativeTime(msUntilStart)} from now)`,
    );

    scheduledTimer = setTimeout(() => {
      debugLog("🚀 Executing scheduled Claude session start");
      vscode.window.showInformationMessage(
        "🚀 Starting scheduled Claude session...",
      );
      callback();
    ...
    vscode.window.showInformationMessage(
      `Claude session scheduled to start at ${timeString}`,
    );
  } catch (error) {
    debugLog(`❌ Failed to schedule session start: ${error}`);
    vscode.window.showErrorMessage(
      `Invalid scheduled start time format: ${config.session.scheduledStartTime}`,
    );
  }
}
