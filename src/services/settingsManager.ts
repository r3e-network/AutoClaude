...
export class SettingsManager {
  ...
  async acquireLock(): Promise<boolean> {
    try {
      ...
      await fsAsync.writeFile(this.lockFile, JSON.stringify(lock), {
        flag: "wx",
      });
      return true;
    } catch (error) {
  ...
  private async writeSettings(settings: any): Promise<void> {
    ...
    await fsAsync.writeFile(tempPath, JSON.stringify(settings, null, 2), {
      mode: 0o600,
    });
  ...
  private async rotateBackups(): Promise<void> {
    ...
    while (totalSize > this.maxBackupSize && backups.length > 1) {
      ...
      if (oldest) {
        ...
        debugLog(
          `Removed backup to stay under size limit: ${path.basename(oldest.filename)}`,
        );
      }
    }
