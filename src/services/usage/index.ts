...
export function isCurrentUsageLimit(output: string): boolean {
  try {
    const usageLimitPattern =
      /(Claude\s+)?usage\s+limit\s+reached.*?reset\s+at\s+(\d{1,2}[:\d]*(?:\s*[APM]{2})?(?:\s*\([^)]+\))?)/gi;
    const matches = [];
    let match;
    ...
    debugLog(
      `🕐 Found ${matches.length} usage limit occurrence(s), checking last one: "${lastMatch.fullMatch}"`,
    );
...
export function handleUsageLimit(output: string, message: MessageItem): void {
  const resetTimeMatch = output.match(
    /reset at (\d{1,2}[:\d]*(?:\s*[APM]{2})?(?:\s*\([^)]+\))?)/i,
  );
  ...
  const existingContinue = messageQueue.find(
    (msg) => msg.text === "continue" && msg.status === "waiting",
  );
  if (existingContinue) {
  ...
  const currentMessageIndex = messageQueue.findIndex(
  ...
  vscode.window.showWarningMessage(
    `Claude usage limit reached. Added "continue" message to queue. Will automatically resume processing at ${resetTime} (${waitMinutes} minutes)`,
  );
...
function startCountdownTimer(message: MessageItem, waitSeconds: number): void {
  ...
  const interval = setInterval(() => {
    ...
    const timeLeft = Math.max(
      0,
      Math.floor((message.waitUntil! - Date.now()) / 1000),
    );
...
function resumeProcessingFromWait(message: MessageItem): void {
  ...
  debugLog(
    `🔄 Resumed processing from wait - message ${message.id} status updated`,
  );

  vscode.window.showInformationMessage(
    'Usage limit has reset. Resuming processing with "continue" message...',
  );
...
export function recoverWaitingMessages(): void {
  ...
  messageQueue.forEach((message) => {
    if (message.status === "waiting" && message.waitUntil) {
      const timeLeft = Math.max(
        0,
        Math.floor((message.waitUntil - now) / 1000),
      );

      if (timeLeft <= 0) {
        debugLog(
          `⏰ Timer expired for message ${message.id} - resuming immediately`,
        );
        resumeProcessingFromWait(message);
      } else {
        debugLog(
          `⏰ Recovering timer for message ${message.id} - ${timeLeft} seconds remaining`,
        );
...
export function simulateUsageLimit(): void {
  ...
  const currentProcessingIndex = messageQueue.findIndex(
  ...
  if (currentProcessingIndex >= 0) {
    ...
    debugLog(
      `🧪 Inserted continue message after processing message at index ${currentProcessingIndex}`,
    );
  } else {
    // If no message is processing, insert at the beginning
    messageQueue.unshift(continueMessage);
    debugLog(
      "🧪 Inserted continue message at beginning of queue (no processing message found)",
    );
  }
  ...
  vscode.window.showInformationMessage(
    'DEBUG: Simulated usage limit. Added "continue" message with 1-minute countdown.',
  );
...
export function clearAllTimers(): void {
  ...
  updateWebviewContent();
  vscode.window.showInformationMessage(
    "DEBUG: Cleared all timers and reset waiting messages to pending.",
  );
}
