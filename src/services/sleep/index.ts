...
import {
  ...
  sleepPreventionProcess,
  setSleepPreventionActive,
  setSleepPreventionProcess,
  claudePanel,
} from "../../core/state";
...
export function startSleepPrevention(): void {
  ...
  debugLog(
    `💤 Sleep prevention setting: ${preventSleep}, already active: ${sleepPreventionActive}`,
  );
  ...
  try {
    ...
    switch (platform) {
      ...
      case "linux":
        ...
        args = [
          "--what=sleep:idle",
          "--who=ClaudeAutopilot",
          "--why=Waiting for Claude usage limit reset",
          "sleep",
          "7200",
        ];
        break;
    ...
    vscode.window.showInformationMessage(
      "Sleep prevention enabled while waiting for Claude usage limit reset",
    );
  } catch (error) {
...
export function sendSleepPreventionSetting(): void {
  ...
  if (claudePanel) {
    try {
      ...
      debugLog(
        `❌ Failed to send sleep prevention setting to webview: ${error}`,
      );
    }
  }
}
