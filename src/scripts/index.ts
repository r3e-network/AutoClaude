...
import {
  SHELL_SCRIPTS as BUILTIN_SCRIPTS,
  DEFAULT_CONFIG,
  SCRIPTS_README,
} from "./shellScripts";
...
export class ScriptRunner {
    ...
    for (const [filename, content] of Object.entries(BUILTIN_SCRIPTS)) {
      ...
      if (!fs.existsSync(scriptPath)) {
        ...
        try {
          ...
          debugLog(
            `Warning: Could not set executable permission for ${filename}: ${error}`,
          );
        }
      }
    }
  ...
  ): Promise<{ allPassed: boolean; results: Map<string, ScriptResult> }> {
  ...
  async runCheckLoop(forceAnalysis: boolean = false): Promise<void> {
    ...
    if (initialCheck.allPassed && !forceAnalysis) {
      ...
      const passingScripts = Array.from(initialCheck.results.entries()).map(
        ([scriptId, _]) => {
          const script = this.config.scripts.find((s) => s.id === scriptId);
      ...
      vscode.window.showInformationMessage(
        "Sent to Claude for improvement analysis",
      );
      return;
    }
    ...
    while (this.iteration < this.config.maxIterations) {
      ...
      const fixInstructions = this.generateFixInstructions(
        initialCheck.results,
      );
      ...
      try {
        ...
        // Show progress
        vscode.window.showInformationMessage(
          `Script check iteration ${this.iteration}: Waiting for Claude to fix issues...`,
        );
        ...
        if (allPassed) {
          vscode.window.showInformationMessage(
            `✅ All checks passed after ${this.iteration} iteration(s)!`,
          );
          return;
        }
        ...
        vscode.window.showErrorMessage(
          `Script check loop error: ${error instanceof Error ? error.message : String(error)}`,
        );
        return;
      }
    }
    ...
    vscode.window.showWarningMessage(
      `⚠️ Maximum iterations (${this.config.maxIterations}) reached. Some checks still failing.`,
    );
    ...
    vscode.window.showErrorMessage(
      `Failed checks: ${finalFailures.join(", ")}`,
    );
  }
  ...
  async runMessageLoop(
    ...
    vscode.window.showInformationMessage(
      "Processing message before running checks...",
    );
    ...
    while (this.iteration < this.config.maxIterations) {
      ...
      if (allPassed) {
        if (iterationCount === 1 && !forceAnalysis) {
          ...
          const passingScripts = Array.from(results.entries()).map(
            ([scriptId, _]) => {
              const script = this.config.scripts.find((s) => s.id === scriptId);
              return script?.name || scriptId;
            },
      ...
      try {
        ...
        // Show progress
        vscode.window.showInformationMessage(
          `Message loop iteration ${this.iteration}: Waiting for Claude to fix issues...`,
        );
        ...
        vscode.window.showErrorMessage(
          `Message loop error: ${error instanceof Error ? error.message : String(error)}`,
        );
        return;
      }
    }
      ...
      if (!result.passed) {
        ...
        if (script) {
          ...
          if (totalLines >= MAX_TOTAL_LINES) {
            ...
            instructions.push(
              `Additional failing scripts: ${Array.from(results.entries())
                .filter(([id, r]) => !r.passed && id !== scriptId)
                .map(
                  ([id]) =>
                    this.config.scripts.find((s) => s.id === id)?.name || id,
                )
                .join(", ")}`,
            );
            break;
          ...
          const errorsToShow = Math.min(
            result.errors.length,
            MAX_ERRORS_PER_SCRIPT,
          );
          ...
          if (result.errors.length > MAX_ERRORS_PER_SCRIPT) {
            instructions.push(
              `- ... and ${result.errors.length - MAX_ERRORS_PER_SCRIPT} more errors`,
            );
            totalLines++;
          }
