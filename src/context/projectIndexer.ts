...
export interface DependencyInfo {
  ...
  go?: {
    modules?: Array<{ path: string; version: string }>;
  };
}
...
export class ProjectIndexer {
  ...
  static getInstance(workspaceRoot: string): ProjectIndexer {
    if (
      ...
      ProjectIndexer.instance.workspaceRoot !== workspaceRoot
    ) {
  ...
  private async loadIndex(): Promise<boolean> {
    try {
      if (fs.existsSync(this.indexFile)) {
        ...
        parsed.codebaseStats.filesByLanguage = new Map(
          parsed.codebaseStats.filesByLanguage,
        );
    ...
    try {
      const toSave = {
        ...
        codebaseStats: {
          ...
          filesByLanguage: Array.from(
            this.index.codebaseStats.filesByLanguage.entries(),
          ),
        },
      };
  ...
  private convertSymbols(symbols: vscode.DocumentSymbol[]): SymbolInfo[] {
    return symbols.map((symbol) => ({
      ...
      children: symbol.children
        ? this.convertSymbols(symbol.children)
        : undefined,
    }));
  }
  ...
  private async analyzeProjectStructure(): Promise<ProjectStructure> {
    ...
    if (this.index?.dependencies.npm) {
      ...
      if (deps["@testing-library/react"])
        structure.testFrameworks.push("React Testing Library");
...
## NPM Dependencies
### Production
${Object.keys(this.index.dependencies.npm.dependencies || {})
  .slice(0, 20)
  .join(", ")}

### Development
${Object.keys(this.index.dependencies.npm.devDependencies || {})
  .slice(0, 20)
  .join(", ")}
`);
    }
    ...
    context.push(`
## Largest Files
${this.index.codebaseStats.largestFiles
  .slice(0, 10)
  .map((file) => `- ${file.relativePath} (${Math.round(file.size / 1024)}KB)`)
  .join("\n")}
`);
...
`,
  ...
  async searchSymbols(query: string): Promise<SymbolInfo[]> {
    ...
    for (const fileInfo of this.index.files.values()) {
      if (fileInfo.symbols) {
        const matchingSymbols = this.searchSymbolsRecursive(
          fileInfo.symbols,
          queryLower,
        );
