...
This project appears to be a ${this.inferProjectType()} application.`,
    ...
    if (stats.frameworks.length > 0) {
      summary.push(
        `It uses the following frameworks: ${stats.frameworks.join(", ")}.`,
      );
    }
  ...
  private async detectDesignPatterns(): Promise<CodePattern[]> {
    ...
    const singletonFiles = await this.searchPattern(
      "getInstance|instance\\s*=\\s*null|private\\s+constructor",
    );
    ...
    const factoryFiles = await this.searchPattern(
      "create[A-Z]\\w+|factory|Factory",
    );
    ...
    const observerFiles = await this.searchPattern(
      "subscribe|unsubscribe|emit|EventEmitter|Observer",
    );
  ...
  private async detectCodingConventions(): Promise<CodePattern[]> {
    ...
    const errorHandling = await this.searchPattern(
      "try\\s*{|catch\\s*\\(|\.catch\\(",
    );
    ...
    // Logging patterns
    const logging = await this.searchPattern(
      "console\\.log|debugLog|logger\\.|log\\(",
    );
    if (logging.length > 0) {
      patterns.push({
  ...
  private async detectArchitecturalPatterns(): Promise<CodePattern[]> {
    ...
    const layers = {
      presentation: await this.searchPattern(
        "ui|UI|view|View|component|Component",
      ),
      business: await this.searchPattern(
        "service|Service|business|Business|logic",
      ),
      data: await this.searchPattern(
        "repository|Repository|dao|DAO|database|Database",
      ),
    };

    if (
      layers.presentation.length > 0 &&
      layers.business.length > 0 &&
      ...
      patterns.push({
        ...
        occurrences: Object.values(layers).reduce(
  ...
  private async generateRecommendations(): Promise<string[]> {
    ...
    if (testRatio < 0.3) {
      recommendations.push(
        "📝 **Increase test coverage**: The test-to-code ratio is low. Consider adding more unit tests.",
      );
    }
    ...
    if (failedTasks.length > 5) {
      recommendations.push(
        "🔧 **Review failed tasks**: Multiple tasks have failed. Consider reviewing error patterns.",
      );
    }
    ...
    if (!readmeExists) {
      recommendations.push(
        "📚 **Add README**: Create a README.md file to document the project.",
      );
    }
    ...
    if (projectStats.dependencies?.npm) {
      ...
      if (totalDeps > 100) {
        recommendations.push(
          "📦 **Review dependencies**: High number of dependencies. Consider auditing for unused packages.",
        );
      }
    }
    ...
    if (largeFiles.length > 5) {
      recommendations.push(
        "🔄 **Refactor large files**: Several files exceed recommended size. Consider breaking them down.",
      );
    }
    ...
    if (complexFiles.length > 0) {
      recommendations.push(
        "⚡ **Optimize complex code**: Some files have high complexity. Consider refactoring for better performance.",
      );
    }
  ...
  private async calculateComplexityMetrics(): Promise<ComplexityMetrics> {
    const files = Array.from(
      this.projectIndexer["index"]?.files.values() || [],
    );
    ...
    const avgComplexity =
      complexityScores.length > 0
        ? complexityScores.reduce((sum, s) => sum + s.complexity, 0) /
  ...
  private async calculateFileComplexity(file: FileInfo): Promise<number> {
    ...
    if (file.symbols) {
      ...
      const countNested = (symbols: Array<{ children?: Array<unknown> }>, depth: number = 0): number => {
        let count = 0;
  ...
  private async calculateCouplingScore(): Promise<number> {
    ...
    const importPatterns = await this.searchPattern(
      "import\\s+.*from|require\\(",
    );
  ...
  private async detectCommonIssues(): Promise<Issue[]> {
    ...
    if (consoleLogFiles.length > 10) {
      issues.push({
        ...
        description:
          "Excessive console.log statements (consider using proper logging)",
    ...
    // Hardcoded values
    const hardcodedFiles = await this.searchPattern(
      "localhost:|127\\.0\\.0\\.1|hardcoded|HARDCODED",
    );
    if (hardcodedFiles.length > 0) {
      issues.push({
        ...
        description:
          "Hardcoded values detected (consider using configuration files)",
  ...
  private inferProjectType(): string {
    ...
    if (
      stats.frameworks.includes("React") ||
      stats.frameworks.includes("Vue") ||
      stats.frameworks.includes("Angular")
    ) {
      return "web frontend";
    }
    if (
      stats.frameworks.includes("Express") ||
      stats.frameworks.includes("Fastify")
    ) {
    ...
    if (
      stats.configFiles.includes("requirements.txt") ||
      stats.configFiles.includes("setup.py")
    ) {
      return "Python";
    }
    if (
      stats.configFiles.includes("pom.xml") ||
      stats.configFiles.includes("build.gradle")
    ) {
      return "Java";
    }
  ...
  private getProjectStats(): Record<string, number | string> {
    ...
    return {
      mainLanguages: Array.from(
        index.codebaseStats.filesByLanguage.keys(),
      ).slice(0, 3),
...
### Most Complex Files
${insights.complexityMetrics.mostComplexFiles
  .slice(0, 5)
  .map((f) => `- ${f.file}: ${Math.round(f.complexity)} complexity points`)
  .join("\n")}
