...
export class ClaudeContextIntegration {
  ...
  private async extractAndTrackResources(message: string): Promise<void> {
    ...
    const commandRegex =
      /(?:^|\s)(?:npm|yarn|git|python|node|cargo|go|make|docker|kubectl)\s+[\w\s\-\.]+/gm;
  ...
  private isValidFilePath(path: string): boolean {
    ...
    return (
      !invalidPatterns.some((pattern) => pattern.test(path)) &&
  ...
  async onSessionEnd(): Promise<void> {
    ...
    try {
      ...
      if (this.messageBuffer.length > 0) {
        const combinedMessage = this.messageBuffer.join("\n");
        this.messageBuffer = [];
        await this.contextProvider.addMessageToCurrentTask(
          combinedMessage,
          "user",
        );
      }
