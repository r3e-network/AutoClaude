...
export class TaskPersistenceManager {
  ...
  static getInstance(workspaceRoot: string): TaskPersistenceManager {
    if (
      !TaskPersistenceManager.instance ||
      TaskPersistenceManager.instance.workspaceRoot !== workspaceRoot
    ) {
      TaskPersistenceManager.instance = new TaskPersistenceManager(
        workspaceRoot,
      );
    }
  ...
  async initialize(): Promise<void> {
    ...
    if (unfinishedTasks.length > 0) {
      debugLog(
        `Found ${unfinishedTasks.length} unfinished tasks from previous sessions`,
      );
    }
  }
  ...
  getTaskHierarchy(
    taskId: string,
  ): { task: PersistedTask; subtasks: PersistedTask[] } | null {
    const task = this.tasks.get(taskId);
    if (!task) return null;
...
`;
  ...
  exportTasksToMarkdown(): string {
    const tasks = this.getAllTasks().sort(
