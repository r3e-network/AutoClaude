...
export class ContextProvider {
  ...
  private contextCache: Map<string, { context: string; timestamp: number }> =
    new Map();
  ...
  static getInstance(workspaceRoot: string): ContextProvider {
    if (
      ...
      ContextProvider.instance.workspaceRoot !== workspaceRoot
    ) {
  ...
  async generateFullContext(
    ...
    if (
      ...
      context.fullContext.length > opts.maxContextLength
    ) {
      context.fullContext = this.truncateContext(
        context.fullContext,
        opts.maxContextLength,
      );
    }
...
\`\`\``;
    ...
    for (const [priority, tasks] of Object.entries(byPriority)) {
      ...
      for (const task of tasks) {
        ...
        if (task.context.errors.length > 0) {
          context.push(
            `- **Recent Errors**: ${task.context.errors[task.context.errors.length - 1]}`,
          );
        }

        if (task.context.files.length > 0) {
          context.push(
            `- **Files**: ${task.context.files.slice(-3).join(", ")}`,
          );
        }
  ...
  async getContextForFile(filePath: string): Promise<string> {
    ...
    // Get related tasks
    const relatedTasks = this.taskManager
      .getAllTasks()
      .filter((task) => task.context.files.includes(filePath));
  ...
  private truncateContext(context: string, maxLength: number): string {
    ...
    if (lastSectionIndex > maxLength * 0.8) {
      return (
        truncated.substring(0, lastSectionIndex) +
