    ...
    describe('ValidationError', () => {
        it('should create InvalidInputError', () => {
            ...
            expect(error.message).toBe(
                "Invalid input for field 'age': expected number, got string"
            );
...
describe('ErrorHandler', () => {
    ...
    describe('getUserMessage', () => {
        it('should return user-friendly messages for known errors', () => {
            expect(errorHandler.getUserMessage(new SessionTimeoutError())).toBe(
                'The session timed out. Please try again.'
            );

            expect(errorHandler.getUserMessage(new SessionNotActiveError())).toBe(
                'The Claude session is not active. Restarting...'
            );

            expect(errorHandler.getUserMessage(new InvalidConfigError('test'))).toBe(
                'Configuration error. Please check your settings.'
            );

            expect(errorHandler.getUserMessage(new CommandInjectionError('test'))).toBe(
                'Invalid command detected for security reasons.'
            );

            expect(errorHandler.getUserMessage(new ConnectionError('service'))).toBe(
                'Connection failed. Please check your network.'
            );

            expect(errorHandler.getUserMessage(new QueueFullError(100))).toBe(
                'Too many pending tasks. Please wait...'
            );

            expect(errorHandler.getUserMessage(new OutOfMemoryError())).toBe(
                'Low memory. Some features may be limited.'
            );
        });

        it('should return default message for unknown errors', () => {
            expect(errorHandler.getUserMessage(new Error('Some error'))).toBe(
